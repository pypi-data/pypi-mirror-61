...
Note the usage of the `-s` flag to use an existing session, instead of creating a new one. Once a valid session is created, it is not required to use a valid password in the command line. However, a `-p` parameter is always required to prevent the script to ask for it interactively.
...
Copyright (C) 2016-2020 Julien EHRHART
