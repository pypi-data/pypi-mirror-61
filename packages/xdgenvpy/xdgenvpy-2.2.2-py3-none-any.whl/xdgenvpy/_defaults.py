...
# Helper that stores the name of the user.  Unfortunately we cannot just just
# os.getlogin() as that is only available while running inside a login-shell.
# Meaning, that will sometimes raise exceptions when running in particular CI
# environments.
_USERNAME = getpwuid(getuid())[0]
...
def XDG_RUNTIME_DIR():
    ...
    """
    return _normalize(linux=f'/run/user/{getuid()}',
                      mac=f'/tmp/run/user/{getuid()}',
                      windows=f'run/user/{_USERNAME}',
                      multiple_dirs=False)
