...
from .base import TestFormatTransformer
...
class NiATMLFormatTransformer(TestFormatTransformer):
    def collect_fn(self, atml_filename):
        return collect_filename(atml_filename)
    
    def transform_fn(self, test_record):
        import pandas as pd
        ...
        start_datetime = pd.to_datetime(result_set.get('@startDateTime'))
        end_datetime = pd.to_datetime(result_set.get('@endDateTime'))
...
def collect_phase(test_phase, accept_all=False):
    ...
    return dict(
        name = name,
        start_datetime = pd.to_datetime(test_phase.get('@startDateTime')),
        end_datetime = pd.to_datetime(test_phase.get('@endDateTime')),
        outcome = make_outcome(test_phase),
        **test_phase
    )
