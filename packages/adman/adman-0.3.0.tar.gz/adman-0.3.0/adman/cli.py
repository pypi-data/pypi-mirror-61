...
@posix_assign
def cmd_allmaint(args, ad, pa):
    cmd_assignids.__wrapped__(args, ad, pa)
    cmd_user_setupns.__wrapped__(args, ad)
    cmd_user_checkexpire.__wrapped__(args, ad)


@posix_assign
def cmd_assignids(args, ad, pa):
...
def cmd_computer_assign(args, ad, pa):
    pa.assign_computer_uidNumbers()
    pa.update_computer_gidNumbers()
...
def cmd_group_assign(args, ad, pa):
    pa.assign_group_gidNumbers(
            include_computer_groups = args.config.id_assign.computers,
        )
...
def cmd_user_assign(args, ad, pa):
    pa.assign_user_uidNumbers()
    pa.update_user_gidNumbers()
