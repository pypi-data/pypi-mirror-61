...
class CfgBase(ClassdataStrMixin):
    def __init__(self, data, name=None, parentnode=None, cfgpath=None):
        self._data = data

        # Root node: name==None, parentnode==None, cfgpath!=None
        # Nonroot:   name!=None, parentnode!=None, cfgpath==None
        nonroot_param_count = sum((x is not None) for x in (name, parentnode))
        if cfgpath is None:
            if nonroot_param_count != 2:
                raise ValueError("Both name, parentnode must be given for nonroot nodes")
        else:
            if nonroot_param_count != 0:
                raise ValueError("name, parentnode must not be given for root node")
    ...
    @property
    def cfgpath(self):
        return self._cfgpath or self._parentnode.cfgpath

    @property
    def keyname(self):
        if self._parentnode:
            return '.'.join((self._parentnode.keyname, self._name))
        else:
            return ''

    def subkeyname(self, name):
        if name:
            return '.'.join((self.keyname, name))
        else:
            return self.keyname
    ...
    def cfg_get_path(self, key):
        """Get a path entry from the config data

        Parameters:
        key     name of the entry to get
        """
        p = Path(self.cfg_get(key))
...
class LdapAuthConfig(CfgBase):
    ...
    def from_mode(data, name, parentnode):
        ...
        o = CfgBase(data=data, name=name, parentnode=parentnode)
...
class GssapiLdapAuthConfig(LdapAuthConfig):
    def __init__(self, data, name, parentnode, mode):
        super().__init__(data=data, name=name, parentnode=parentnode, mode=mode)
...
class UpnSuffixConfig(CfgBase):
    def __init__(self, data, name, parentnode):
        ...
        if isinstance(data, str):
            ...
            parentnode.raise_for_key(name, "Invalid type: {}".format(type(data).__name__))

        if not self.scope in ('subtree', 'one'):
            self.raise_for_key('scope', "Invalid scope: {}".format(self.scope))


class PasswordExpConfig(CfgBase):
    def __init__(self, data, name, parentnode):
        super().__init__(data=data, name=name, parentnode=parentnode)
        ...
        if isinstance(days, int):
            ...
            self.raise_for_key('days', "Invalid type: {}".format(type(days).__name__))
