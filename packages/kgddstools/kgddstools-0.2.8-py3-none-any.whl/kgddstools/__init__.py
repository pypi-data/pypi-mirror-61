...
def plot_cont(input_df_cont, cont_cols):
    print(cont_cols)

def plot_cat(input_df_cat, cat_cols):
    print(cat_cols)
...
def eda(input_df, target_var, type, cat = [], cont = []):
    ...
    if type == 'df_summary':
        ...
        if(len(cat) >= 1):
            ...
            plot_cat(input_df_cat = input_df, cat_cols = cat)

        if(len(cont) >= 1):
            print('cont var')

            plot_cont(input_df_cont = input_df, cont_cols = cont)
