...
def mean_stb_data (X):
    X_mean = X.mean()
    X_std = X.std()
    X_up = X_mean + 0.5 * X_std
    X_low = X_mean - 0.5 * X_std
...
def spec_plot_binary(df,
    ...
    wavenumbers = df.loc[:, "1801.264": "898.703"].columns.values.astype("float")
    X = df.loc[:, "1801.264":"898.703"]
    y = df[label]

    df_sub = pd.concat([X, y], axis=1)
    df_sub = df_sub.dropna()
    spectra_1 = df_sub[df_sub[label] == class_1]
    spectra_1 = pd.DataFrame(spectra_1.drop(label, axis=1).values.astype("float"))

    spectra_2 = df_sub[df_sub[label] == class_2]
    spectra_2 = pd.DataFrame(spectra_2.drop(label, axis=1).values.astype("float"))

    plt.rc("font", size=16, family="Times New Roman")
    plt.rc('axes', linewidth=2)
    fig = plt.figure(figsize=(12, 6))
    ax1 = fig.add_axes([0, 0, 1, 1])
    ax2 = fig.add_axes([0.02, 0.43, 0.75, 0.54])
    ax2.set_yticks([])

    _, spec_1_mean, spec_1_up, spec_1_low = mean_stb_data(spectra_1)
    ax1.plot(wavenumbers, spec_1_mean, color="blue")
    ax1.fill_between(wavenumbers, spec_1_up, spec_1_low, color="blue", alpha=0.1)

    ax2.plot(wavenumbers, spec_1_mean, color="blue", label=label_1)
    ax2.fill_between(wavenumbers, spec_1_up, spec_1_low, color="blue", alpha=0.1)

    _, spec_2_mean, spec_2_up, spec_2_low = mean_stb_data(spectra_2)
    ax1.plot(wavenumbers, spec_2_mean, color="red")
    ax1.fill_between(wavenumbers, spec_2_up, spec_2_low, color="red", alpha=0.1)

    ax2.plot(wavenumbers, spec_2_mean, color="red", label=label_2)
    ax2.fill_between(wavenumbers, spec_2_up, spec_2_low, color="red", alpha=0.1)

    if functional_groups:
        ax1.annotate('Amid III', xy=(1241, 0.028), xytext=(1241, 0.05),
                     arrowprops=dict(facecolor='black', shrink=0.05, width=2))

        ax1.annotate('as CH' + r"$_3$", xy=(1456, 0.018), xytext=(1456 - 8, 0.045),
                     arrowprops=dict(facecolor='black', shrink=0.05, width=2))

        ax1.annotate("Amid II", xy=(1548, 0.12), xytext=(1548 - 27, 0.16),
                     arrowprops=dict(facecolor='black', shrink=0.05, width=2))

        ax1.annotate("Amid I", xy=(1659, 0.232), xytext=(1659 - 25, 0.245),
                     arrowprops=dict(facecolor='black', shrink=0.05, width=2))

        ax1.annotate("sym CCO" + r"$^-$", xy=(1400, 0.02), xytext=(1400, 0.058),
                     arrowprops=dict(facecolor='black', shrink=0.05, width=2))

    ax1.set_xlabel("Wavenumber (cm" + r"$^{-1})$", fontdict=dict(size=23, fontname="Times New Roman", fontweight='bold'))
    ax1.set_ylabel("Absorbance (A.U)", fontdict=dict(size=23, fontname="Times New Roman", fontweight='bold'))
    ...
    ax2.text(1135, 0.08, s=text_name, fontdict=dict(size=22, fontname="Times New Roman"))

    if graph_item != "none":
        ax1.text(1755, 0.23, s="(" + graph_item + ")", fontdict=dict(size=25, fontname="Times New Roman", fontweight="bold"))

    if save_fig:
        fig.savefig(text_name + ".tif", dpi=600, bbox_inches='tight')
