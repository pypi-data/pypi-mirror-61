...
def create_plots(methods=["RVM"], dataN=1000):
    ...
    for i in range(tester.dim):
        for j in range(i+1, tester.dim):
            ...
            #create_plot(tester, 0, [i, j], baseName+str([i,j]))
    #"""
#line_profile = line_profiler.LineProfiler(RK4ag)
#line_profile.runcall(g, x0)
#line_profile.print_stats()
#line_profile.runcall(h, x0)
#line_profile.print_stats()
...
if __name__ == '__main__':
    ...
    #methods = ["bisection"]
    benchmark(methods, 200, dataN=1000, mode="11") 
    sys.exit()
    benchmark(methods, 200, dataN=10000, mode="11") 
    create_plots(["RVM"], 500)
    test_LogRegress(methods)
    create_plots(methods)
