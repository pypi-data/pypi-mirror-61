...
def set_data_with_buffer(func: callable, buffered_argument: str):
    ...
    def buffered_arguments(args, kwargs):
        ...
        if any(len(str(item)) > 4000 for item in buffer):
            raise ValueError("One of the arguments to {} is over the 4K "
        ...
        while offset < buffer_size:
            ...
            while sum(len(str(s))
                      for s in buffer[offset:current_end + 1]) < 4000\
...
def delete(datastore: str, keys: list, customer: get_customer_id = None):
    ...
    """
    buffered_delete = set_data_with_buffer(delete_data_store_entries,
                                           buffered_argument="key")
    result = buffered_delete(dataStore=datastore, customerID=customer, key=keys)

    print("Successfully deleted {amount} entries"
          .format(amount=result["count"]))
