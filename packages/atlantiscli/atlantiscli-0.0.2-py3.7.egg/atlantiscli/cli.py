...
@click.pass_context
def issue_token(ctx, code, client_id, client_secret):
    click.secho("   Issuing token...", fg='green')
    token = ctx.obj.client.issue_token(code, client_id, client_secret)
    click.secho(json.dumps(token, indent=2, sort_keys=True), fg='yellow')
