...
def enum_map(
    ...
    enum member must have a tuple value with a length equal to the number of keywords defined in the `keys` parameter.

    For each key in the `keys` parameter, a method called `to_<key>` and a class method called `from_<key>` will be
    added to the enum. These convert to and from the enum values at the index of the keyword. For example, given a
    mapped enum `Events` created with keys `'day month'` and a single member `BIRTHDAY = 11, 'august'`, a call to
    `Events.BIRTHDAY.to_day()` would return `11` and `Events.BIRTHDAY.to_month()` would return `'august'`. Likewise, a
    call to `Events.from_day(11)` would return `Events.BIRTHDAY`. Details on the generated methods are provided below.
    ...
    :param allow_override: If True, if the enum defines methods with the same name as a possible method generated by
    `enum_map`, the generated methods will have a traililng underscore, or a number of trailing underscores if there
    also already exists conflicting names with underscores. This allows for performing special logic for certain
    members of the enum, but falling back to the generated method for the remainder. If False, then an error will be
    thrown when the Enum has a method conflicting with a generated one.
