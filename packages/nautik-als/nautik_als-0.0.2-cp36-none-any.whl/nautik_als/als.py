...
class AlternatingLeastSquares(MatrixFactorizationBase):
    ...
    def __init__(self, user_features=None, item_features=None, factors=100, 
        ...
        # features approximation matrices
        self.user_features_approx = None
        self.item_features_approx = None
    ...
    def fit(self, item_users, show_progress=True):
        # TODO: User und Item Features mitgeben
        ...
        if Ciu.dtype != np.float64:
            Ciu = Ciu.astype(np.float64)
            
        if self.user_features.dtype != np.float64:
            self.user_features = self.user_features.astype(np.float64)    

        if self.item_features.dtype != np.float64:
            self.item_features = self.item_features.astype(np.float64)    
        ...
        if self.user_features_approx is None:
            self.user_features_approx = np.random.rand(n_user_features, self.factors).astype(self.dtype) * 0.01
        if self.item_features_approx is None:
            self.item_features_approx = np.random.rand(n_item_features, self.factors).astype(self.dtype) * 0.01
        ...
        with tqdm(total=self.iterations, disable=not show_progress) as progress:
            ...
            for iteration in range(self.iterations):
                s = time.time()
                solver(Cui, self.user_features, self.user_factors, self.item_factors, 
                       self.user_features_approx, self.regularization,
                       num_threads=self.num_threads)
                s1 = time.time()
                print('Iteration {}, approximate X: {} min.'.format(iteration, round(int(s1 - s) / 60, 1)))
                solver(Ciu, self.item_features, self.item_factors, self.user_factors, 
                       self.item_features_approx, self.regularization,
                       num_threads=self.num_threads)
                s2 = time.time()
                print('Iteration {}, approximate Y: {} min.'.format(iteration, round(int(s2 - s1) / 60, 1)))
                feature_least_squares(self.user_features, self.user_factors, 
                                      self.user_features_approx, self.regularization)
                s3 = time.time()
                print('Iteration {}, approximate user features: {} min.'.format(iteration, round(int(s3 - s2) / 60, 1)))
                feature_least_squares(self.item_features, self.item_factors, 
                                      self.item_features_approx, self.regularization)
                s4 = time.time()
                print('Iteration {}, approximate item features: {} min.'.format(iteration, round(int(s4 - s3) / 60, 1)))
                progress.update(1)
...
def feature_least_squares(F, X, S, regularization):
    features, factors = S.shape
    xTx = X.T.dot(X)
    A = xTx + regularization * np.eye(factors, dtype=X.dtype)
    
    for f in range(features):
        b = X.T.dot(F.T[f])
        S[f] = np.linalg.solve(A, b)   

def least_squares_cg(Cui, F_X, X, Y, S, regularization, num_threads=0, cg_steps=3):
    
    print('X.shape: {}'.format(X.shape))
    print('F_X.shape: {}'.format(F_X.shape))
    print('Y.shape: {}'.format(Y.shape))
    print('S.shape: {}'.format(S.shape))
    print('Cui.shape: {}'.format(Cui.shape))
    
    users, factors = X.shape
    
    YtY = Y.T.dot(Y) + S.T.dot(S) + regularization * np.eye(factors, dtype=Y.dtype)

    for u in range(users):
        # start from previous iteration
        x = X[u]
        f = F_X[u] # features of user u
        
        # calculate residual error r = (YtCuPu + StFu - ((YtCuY + StS).dot(Xu)))
        r = -YtY.dot(x) + S.T.dot(f)
        for i, confidence in nonzeros(Cui, u):
            if confidence > 0:
