...
class Domain(metaclass=ParametricSingleton):
    __tunable__ = ['inherit_recipients', 'aliases', 'comment']

    def __init__(self, harpo, name, parent,
                 secrets_manager=None, allow=None, deny=None,
                 aliases=None, comment=None, inherit_recipients=True, **_):
        ...
        self.logger.debug("%s initialized: %s", self.__class__, self.__dict__)
    ...
    def _entities_to_users(entities_list):
        ...
        for entity in entities_list:
            if type(entity) is User:
                ...
                raise HarpoException("Unexpected recipient type: '{}'! This should never happen!"
                                     .format(type(entity)))
        return result
    ...
    def reencrypt(self, recurse=True):
        ...
        if recurse:
            self.logger.debug("Reencrypting children of %s: %s", self.name, self.children)
            for child in self.children:
                if child.inherit_recipients:
                    self.logger.info("Triggering re-encryption for domain '%s' (recurse=True)...", child.name)
                    child.reencrypt()
                else:
                    self.logger.debug("Skipping reencryption of domain '%s' because it doesn't inherit recipients",
                                      child.name)
        else:
            self.logger.debug("Skipping reencryption of children domains (recurse=False)")
