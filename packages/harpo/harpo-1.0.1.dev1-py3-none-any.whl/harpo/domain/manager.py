...
class DomainManager(metaclass=ParametricSingleton):
    ...
    @with_lock
    def create(self, domain_name: str, inherit_recipients: bool=True):
        ...
        if len(splitted_domain_name) > 1:
            ...
            parent = None
        self.logger.debug("Domain parent: %s", parent)
    ...
    def _domain_from_metadata(self, metadata):
        ...
        try:
            self.logger.debug("Back-populating children for %s: append %s", parent_domain, domain)
            if domain not in parent_domain.children:
                parent_domain.children.append(domain)
            self.logger.debug("%s children: %s", parent_domain, parent_domain.children)
        except NameError:
            pass
        return domain

    def __getitem__(self, domain_name: str):
        try:
