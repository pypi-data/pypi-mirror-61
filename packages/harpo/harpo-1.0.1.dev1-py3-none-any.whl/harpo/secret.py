...
from harpo.logging import get_logger
...
class SecretPlaintextBackend(SecretBackend):
    ...
    def __init__(self, harpo, domain_metadata):
        ...
        self.logger.debug('%s initialized: %s', self, self.__dict__)
    ...
    def list(self):
        return [os.path.basename(file) for file in glob.glob(os.path.join(self.data_dir, '*'))
...
class SecretGPGBackend(SecretBackend):
    ...
    def decrypt(self, secret_name):
        ...
        gpg = gnupg.GPG(gnupghome=self.harpo.gpg_home)
        return gpg.decrypt(ascii_encrypted_data)
    ...
    def _build_recipients_list(self, shared=False):
        ...
        if shared:
            self.logger.debug("This is a shared secret!")
            chld_queue = list()
    ...
    def encrypt(self, secret_name: str, secret_value: str):
        ...
        self.logger.debug("Encrypt %s::%s for recipients: %s", self.domain_name, secret_name, recipients)
        key_fingerprints = [recipient.key_fingerprint for recipient in recipients]
        self.logger.debug("Encrypting with keys: %s", key_fingerprints)
        ...
        if encrypted_ascii_data.ok:
            self.logger.debug("Successfully encrypted %s::%s. Writing...", self.domain_name, secret_name)
        else:
        ...
        with open(secret_file_path, 'w') as file:
            self.logger.debug('Secret file: %s', secret_file_path)
...
class SecretManager(metaclass=ParametricSingleton):
    def __init__(self, harpo, domain_metadata):
        self.logger = get_logger()
        self.domain_name = domain_metadata['name']
        self.harpo = harpo
    ...
    def _initialize_backend(self):
        ...
        self.logger.debug('Domain %s has selected backend: %s', self.domain_name, type)
