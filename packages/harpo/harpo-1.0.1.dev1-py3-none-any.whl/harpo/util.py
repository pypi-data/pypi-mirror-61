...
import harpo
from harpo.exceptions import HarpoLockFileExists
...
class ParametricSingleton(type):
    ...
    def __call__(cls, *args, **kwargs):
        logger = get_logger()
        if cls in [harpo.domain.domain.Domain, harpo.user.user.User, harpo.group.group.Group]:
        ...
        logger.debug("args: %s, kwargs: %s, hash: %s", args, kwargs, params_hash)
        full_name = "{}:{}".format(cls, params_hash)
        if full_name not in cls._instances:
            logger.debug("New ParametricSingleton instance for %s: %s, %s => %s",
                         cls, str(args), str(kwargs), params_hash)
            cls._instances[full_name] = super(ParametricSingleton, cls).__call__(*args, **kwargs)
        logger.debug("Reusing existing ParametricSingleton instance: %s", full_name)
...
class Singleton(type):
    ...
    def __call__(cls, *args, **kwargs):
        ...
        full_name = cls
        if full_name not in cls._instances:
            logger.debug("New Singleton instance for %s: %s, %s => %s", cls, str(args), str(kwargs))
...
def with_lock(func):
    ...
    def wrapper(*args, **kwargs):
        ...
        try:
            logger.debug("Acquiring lock: %s", lock_file)
            with lock:
                logger.debug("Lock acquired: %s", lock.is_locked)
