...
from harpo.util import ParametricSingleton, to_pretty_json


class MetaData(metaclass=ParametricSingleton):
    ...
    def _verify_metadata_dir(self):
        self.logger.debug("Verifying metadata directories")
        ...
        self.logger.debug("Metadata directories - OK")
        return True

    def _verify_metadata_storage_version(self):
        self.logger.debug("Verifying metadata storage version")
        try:
            ...
            self.logger.debug("Metadata storage version - OK")
        except HarpoOSError as exc:
            raise HarpoMetadataReadError("Failed to read Harpo metadata") from exc
        return True

    def _verify_metadata(self):
        self.logger.debug("Verifying metadata")
        self._verify_metadata_dir()
        self._verify_metadata_storage_version()
        self.logger.debug("Metadata - OK")

    def _get_metadata_file(self, type: str) -> str:
    ...
    def _read_metadata_file(self, type: str) -> dict:
        metadata_filename = self._get_metadata_file(type)
        self.logger.debug("Reading metadata in %s", metadata_filename)
        try:
            with open(metadata_filename, 'r') as json_file:
                json_data = json_file.read()
                data = json.loads(json_data)
        except json.decoder.JSONDecodeError as exc:
            raise HarpoMetadataParsingError(metadata_filename) from exc
        except FileNotFoundError as exc:
            raise HarpoOSError(exc) from exc
        self.logger.debug("Read metadata: %s", data)
        return data

    def _write_metadata_file(self, type: str, data: dict):
        metadata_filename = self._get_metadata_file(type)
        self.logger.debug("Writing metadata to %s", metadata_filename)
        with open(metadata_filename, 'w') as metadata_file:
            metadata_file.write(to_pretty_json(data))
        self.logger.debug("Wrote metadata: %s", data)
