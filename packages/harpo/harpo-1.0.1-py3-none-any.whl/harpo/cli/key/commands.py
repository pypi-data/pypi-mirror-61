...
@with_harpo_context
def create(harpo: Harpo, common_parameters, key_material):
    """Add a new key to Harpo keychain, use '-' to read from STDIN"""
    if key_material == '-':
        key_material = sys.stdin.read()
    print_formatted(harpo.key_create(key_material).results, output_format=common_parameters['format'])


@key.command()
@click.argument('fingerprint')
@with_harpo_context
def destroy(harpo: Harpo, common_parameters, fingerprint):
