...
from harpo.cli.group.commands import complete_group_name
...
@domain.command()
@click.option('--no-inherit-recipients', is_flag=True, help='This domain should not inherit recipients from its parent')
@click.argument('domain_name', autocompletion=complete_domain_name)
@with_harpo_context
def create(harpo: Harpo, common_parameters, domain_name: str, no_inherit_recipients: bool):
    """Create a new domain"""
...
@click.argument('domain_name', autocompletion=complete_domain_name)
@with_harpo_context
def destroy(harpo: Harpo, common_parameters, domain_name: str):
...
@domain.command()
@with_harpo_context
def list(harpo: Harpo, common_parameters):
...
@click.argument('domain_name', autocompletion=complete_domain_name)
@with_harpo_context
def info(harpo: Harpo, common_parameters, domain_name: str):
...
@with_harpo_context
def allow(harpo: Harpo, common_parameters, domain_name: str, entity: str):
...
@click.argument('entity', autocompletion=complete_user_and_group)
@with_harpo_context
def deny(harpo: Harpo, common_parameters, domain_name: str, entity: str):
