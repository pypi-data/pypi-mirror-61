...
class KeyManager(metaclass=ParametricSingleton):
    """Manages GPG keys"""
    ...
    def __getitem__(self, keys: str):
        ...
        if len(keys) > 1:
            self.logger.warning("Multiple keys were found using search term '%s'"
                                "Try specifying an exact key fingerprint", keys)
            return None
        return keys
