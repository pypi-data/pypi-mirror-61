...
class UserManager(metaclass=ParametricSingleton):
    ...
    def exists(self, user_name: str):
        return user_name in self.metadata.users.keys()

    @with_lock
    def create(self, user_name: str, key_fingerprint: str):
        ...
        self.logger.info("Creating user '%s' identified by key with fingerprint '%s'", user_name, key_real_fingerprint)
        user_metadata = {
            "name": user_name,
    ...
    @with_lock
    def destroy(self, user_name: str):
    ...
    def __getitem__(self, user_name: str):
        try:
