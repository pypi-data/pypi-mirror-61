...
class Harpo(object):
    ...
    def domain_create(self, domain_name, inherit_recipients: bool=True):
    ...
    def group_exclude_user(self, group_name, user_name, reencrypt_on_change=True):
        if not self.groups.exists(group_name):
            raise HarpoGroupNotFound("Can't remove user '{}' from a non-existing group '{}'".format(user_name, group_name))
