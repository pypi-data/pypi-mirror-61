...
from .threads import connectionHandler, missionUpdateHandler
...
class NodeLink:
    ...
    def __init__(self, missionUpdateCallback=None, manualControlCallback=None):
        ...
        # Start a thread to frequently check for mission updates from the API
        if missionUpdateCallback is not None:
            self.missionUpdateThread = threading.Thread(target=missionUpdateHandler, args=(lambda:self.threadsActive, missionUpdateCallback, API_URL), daemon=True)
            self.missionUpdateThread.start()
    ...
    # Image (array)
    async def transmitImage(self, image, timeStamp=None, feed='PRIMARY'):
        '''
    ...
    async def disconnect(self):
        ...
        if self.missionUpdateThread is not None:
            self.missionUpdateThread.join()
