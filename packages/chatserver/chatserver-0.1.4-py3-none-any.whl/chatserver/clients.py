...
class AnsiClient(Client):
    async def send_server_message(self, message):
        await self.send(u"\u001b[s\u001b[1G\u001b[2K")  # save position, go to start or 1 row up
        await self.send(message)
        await self.send(u"\u001b[u\u001b[1S")  # resotre position and scroll
    ...
    def format_message(room, message, sender):
        for username in room:  # for mentions
            ...
            if m:
                ...
                message = message[:start] + ansi.ANSI_RESET +  room[username].color + m.group(0) + sender.color + message[end:]
    ...
    async def input(self):
        ...
        await self.send(u"\u001b[1A\u001b[2K")  # go back up (to were inpupt is) and the clear line
        return data
