...
class Sprite():
    ...
    The Flip Sprite - a generic two-dimensional object that is integrated with other sprites in a larger scene. A sprite
    can consist of a bitmap (still) image or a basic geometrical shape (circle, rectangle, ellipse). Sprites in Flip
    ...
    def _update_gravity(self):
       ...
       #print('cover {} ==> next {}'.format(cover_area, next_area))
       clear = True
       for p in next_area:
       ...
       # - stop
       #if moving and direction_y == 1 and clear is False:
       #      if ani is not None:
       #          print('aboring {} at ({},{})'.format(ani.attributes,self.x,self.y))
       #          Globals.instance.animations.remove(ani)
