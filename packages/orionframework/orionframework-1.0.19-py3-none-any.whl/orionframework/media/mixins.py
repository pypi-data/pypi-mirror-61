...
class MediaViewMixin(object):
    ...
    @action(detail=True, methods=["delete"], url_path="media/(?P<media_id>[\d+]+)/(?P<media_type>[-\w]+)")
    ...
    @action(detail=True, methods=["put"], url_path="media",
    ...
    @action(detail=True, methods=["patch"], url_path="media/(?P<media_id>[\d+]+)/(?P<media_type>[-\w]+)")
...
class MediaCategorizedViewMixin(MediaViewMixin):
    @action(detail=True, methods=["put"], url_path="media/(?P<category>[^/.]+)",
            parser_classes=(FormParser, MultiPartParser, FileUploadParser))
