...
class Site:
    ...
    def register_collection(self, collection_cls: typing.Type[Collection]) -> None:
        ...
        self.collections[collection.__class__.__name__] = collection
    ...
    def render(self, dry_run: bool = False) -> None:
        for page in self.routes:
            ...
            page_vars = vars(page)
            page_vars.update(vars(self)) # Adds the local 

            logging.info(page_vars.get('posts', ''))
            content = engine.render(page, **page_vars)
