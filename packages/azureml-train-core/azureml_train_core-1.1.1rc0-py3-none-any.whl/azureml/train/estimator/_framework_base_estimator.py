...
class _FrameworkBaseEstimator(MMLBaseEstimator):
    ...
    def _setup_environment(self, custom_docker_image, compute_target, entry_script, environment_definition,
        ...
        in_mooncake_region = (isinstance(compute_target, AmlCompute) and
                              compute_target.location in _MOONCAKE_REGIONS)
        if compute_target and (in_mooncake_region or not isinstance(compute_target, AmlCompute)):
            self._use_framework_image = False
            if self._optimized_mode:
                if in_mooncake_region:
                    self._optimized_mode = False
                    module_logger.warning("Optimized mode is not currently supported in Azure region "
                                          "chinaeast2. A full image build will be used for this run.")
