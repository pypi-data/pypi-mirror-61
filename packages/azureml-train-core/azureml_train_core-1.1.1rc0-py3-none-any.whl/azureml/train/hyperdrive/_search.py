...
def search(hyperdrive_config, workspace, experiment_name, **kwargs):
    ...
    if hyperdrive_config._resume_from is not None and len(hyperdrive_config._resume_from) > 0:
        ...
        if _current_run_host != _resume_from_sample_run["run_scope"]["host"]:
            raise UserErrorException(("The new run should have the same host as the resume_from runs. The new "
                                     "run's host is '{}', while the resume_from runs' host is '{}'.").format(
                                         _current_run_host,
                                         _resume_from_sample_run["run_scope"]["host"]))
    ...
    if hyperdrive_config._resume_child_runs is not None and len(hyperdrive_config._resume_child_runs) > 0:
        ...
        if _current_run_host != _resume_child_runs_sample["run_scope"]["host"]:
            raise UserErrorException(("The new run should have the same host as resume_child_runs. The new "
                                     "run's host is '{}', while the resume_child_runs' host is '{}'.").format(
                                         _current_run_host,
                                         _resume_child_runs_sample["run_scope"]["host"]))
