...
def test_com_send_command_unsafe(com_config):
    com = NewportLoopSerialCommunication(com_config)
    com.open()
    ...
    cmd = "command"
    param = 32.5
    com._send_command_unsafe(add, cmd, param)
    assert com.get_written() == f"{add}{cmd}{param}"


def test_com_query_unsafe(com_config):
    com = NewportLoopSerialCommunication(com_config)
    com.open()
    # check that the correct answer is returned
    com.put_text("1query answer")
    assert com._query_unsafe(1, "query", "?") == "answer"
...
def test_com_check_for_error_unsafe(com_config):
    com = NewportLoopSerialCommunication(com_config)
    com.open()
    # check that no error is raised
    com.put_text(f"1TE{com.ControllerErrors.NO_ERROR.value}")
    com._check_for_error_unsafe(1)
