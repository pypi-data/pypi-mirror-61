...
class SoundNet:
    ...
    def run(self, sample_rate=16000):
        record_path = join(tempfile.gettempdir(), str(time.time()) + ".wav")
        CHUNK = 1024
        FORMAT = pyaudio.paInt16
        CHANNELS = 1
        p = pyaudio.PyAudio()
        ...
        while self.running:
            audio = None
            # TODO rolling buffer of 6 sec windows instead of reseting buffer
            for i in range(0, int(sample_rate / CHUNK * 2)):
                if audio is None:
                    audio = stream.read(CHUNK)
                else:
                    audio += stream.read(CHUNK)
            ...
            # audio = np.frombuffer(audio)
            # audio = self.preprocess(audio)
            # print(self.model.predict(audio))
