...
class PollSocketWorker(threading.Thread):
    ...
    def __init__(self, thread_name: str, socket: pyzmq.Socket, rx_subject: rx.subject.Subject) -> None:
...
class ServerConnectivity:
    ...
    def __init__(self, address: str, dealer_port: int, subscribe_port: int, receive_timeout_ms: int = 500) -> None:
        ...
        self.zmq_context = pyzmq.Context()
        self.socket_dealer = self.zmq_context.socket(pyzmq.DEALER)
        self.socket_dealer.RCVTIMEO = receive_timeout_ms
        self.socket_subscr = self.zmq_context.socket(pyzmq.SUB)
