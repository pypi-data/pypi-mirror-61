...
def test_protocol_consitency():
    ...
    nose.tools.eq_(len(_make_samples('train')), 8)
    nose.tools.eq_(len(_make_samples('test')), 20)
    # None should return train + test
    nose.tools.eq_(len(_make_samples()),28)
...
def test_paths():
    ...
    for s in samples:
        ...
        assert 'HO' in s.gt.path


@rc_variable_set('bob.db.chasedb1.datadir') 
def test_pils():
    ...
    for s in samples:
        # check size
        nose.tools.eq_(s.img.size,(999, 960))
        nose.tools.eq_(s.gt.size,(999, 960))
        # check modes
...
@rc_variable_set('bob.db.chasedb1.datadir') 
def test_checkfiles():
    """ Test bob_dbmanage.py chasedb1 checkfiles command """

    from bob.db.base.script.dbmanage import main
    nose.tools.eq_(main('chasedb1 checkfiles --self-test'.split()), 0)


@rc_variable_set('bob.db.chasedb1.datadir') 
def test_create():
    """ Test bob_dbmanage.py chasedb1 create command """

    from bob.db.base.script.dbmanage import main
    nose.tools.eq_(main('chasedb1 create --self-test'.split()), 0)