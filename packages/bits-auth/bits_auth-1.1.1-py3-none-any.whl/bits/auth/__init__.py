...
class Auth(object):
    ...
    def _resolve_secrets(self, params):
        """Resolves Secret Manager secrets included in a dict."""
        for k, v in params.items():
            if isinstance(v, str) and v.startswith('sm:///'):
                print('Resolving {}...'.format(v))

                # get the secret id from the url
                secret_id = v.replace('sm:///', '')

                # connect to secret manager client
                if not self.smclient:
                    self.smclient = secretmanager_v1beta1.SecretManagerServiceClient()

                # get the secret name
                name = self.smclient.secret_version_path(
                    self.smproject,
                    secret_id,
                    'latest'
                )
                ...
                response = self.smclient.access_secret_version(name)
