...
class ClusterManager(object):
    ...
    def clean_services(self):
        if "services" not in self.cluster_config:
            return {}
        services = deepcopy(self.cluster_config["services"])
        for svc, val in self.cluster_config["services"].items():
            if "image" not in val.get("values", {}):
                continue
            del val["values"]["image"]
            val["values"].pop("updated_at")
            val["values"].pop("updated_by")
            # we don't want empty values floating around either
            if not val["values"]:
                del val["values"]
        fe = self.cluster_config["frontend"].pop("hash")
        return {"frontend": {"hash": fe}, "services": services}
    ...
    def edit_cluster_config(self, changes=None, version=None):
        """
        Take a patch for cluster config and apply
        """
        if changes:
            self.cluster_config = merge_dicts(self.cluster_config, changes)
        if version:
    ...
    def reconcile(self):
        ...
        self.edit_cluster_config(
            changes={
                "frontend": {"hash": get_deployed_frontend()},
                "services": {
