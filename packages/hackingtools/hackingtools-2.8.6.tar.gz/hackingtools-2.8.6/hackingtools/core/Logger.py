...
def printMessage(message, description=None, debug_module=False, debug_core=False, is_error=False, is_warn=False, is_info=False, is_success=False, color=None):
    ...
    lineNumber = __inspect.currentframe().f_back.f_lineno
    ...
    if description:
        msg = '{methodCalledFrom} - #{lineNumber} - {message} - {description}'.format(methodCalledFrom=methodCalledFrom, lineNumber=lineNumber, message=message, description=description)
    else:
        msg = '{methodCalledFrom} - #{lineNumber} - {message}'.format(methodCalledFrom=methodCalledFrom, lineNumber=lineNumber, message=message)
    ...
    if ((not debug_module == True) and (not debug_core == True) and (__DEBUG_USER__ == True)) or ((debug_module == True) and (__DEBUG_MODULE_FLAG__ == True)) or ((debug_core == True) and (__DEBUG_CORE_FLAG__ == True)):
        if description:
            print('{timeColorStart}[{time}]{timeColorEnd} - {methodCalledFromColorStart}{methodCalledFrom}{methodCalledFromColorEnd} - #{messageColorStart}{lineNumber}{messageColorEnd} - {messageColorStart}{message}{messageColorEnd} - {description}'.format(
            ...
            print('{timeColorStart}[{time}]{timeColorEnd} - {methodCalledFromColorStart}{methodCalledFrom}{methodCalledFromColorEnd} - #{messageColorStart}{lineNumber}{messageColorEnd} - {messageColorStart}{message}{messageColorEnd}'.format(
                timeColorStart=__Fore.BLUE, 
                timeColorEnd=__Fore.WHITE, 
                time=time_now,
                methodCalledFromColorStart=__Fore.GREEN, 
