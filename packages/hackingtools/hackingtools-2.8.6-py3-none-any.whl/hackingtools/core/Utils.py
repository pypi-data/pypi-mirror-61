...
def getAnyFunctionParams(functionObjectStr, i_want_list=False):
    try:
        params_func = __inspect.getfullargspec(eval(functionObjectStr))[0]
        params_func = [param for param in params_func if not param in __function_param_exclude__] if params_func else []

        args, _, __, defaults = __inspect.getargspec(eval(functionObjectStr))
...
def doesFunctionContainsExplicitReturn(functionCall):
    try:
        return True if 'return' in __inspect.getsource(eval(functionCall)) else False
    except:
        return False
