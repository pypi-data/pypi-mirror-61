...
__path = os.path.abspath(os.path.split(os.path.dirname(__file__))[0])
...
def randomText(length=8, alphabet='lalpha', try_pattern=None, pattern_force_chars=None):
    try:
        ...
        return ''.join(random.SystemRandom().choice(__config_utils__[alphabet]) for _ in range(int(length)))
