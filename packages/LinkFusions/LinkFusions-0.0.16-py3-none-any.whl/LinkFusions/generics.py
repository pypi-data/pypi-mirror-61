from .exceptions import BadRequest, ExpiredTokenSessionException
...
class CrudEntity(object):
        ...
        for key, value in data_dict.items():
            try:
                ...
                if type(value) == tuple and hasattr(value[1], 'read'):
                    ...
                    files.update({key: (file.name, file.read())})
                else:
    ...
    def handle_request(self, requestor, as_json=True):
        ...
        try:
            ...
            if self.token and 'Authentication credentials were not provided' in str(e):
                raise ExpiredTokenSessionException('Your token is expired or invalid, please re-initialize the '
                                                   'LinkFusions instance.')
            raise e
