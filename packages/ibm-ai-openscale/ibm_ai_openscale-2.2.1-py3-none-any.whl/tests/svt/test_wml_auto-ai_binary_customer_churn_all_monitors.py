...
from utils.wml_deployments.auto_ai import CustomerChurn
...
class TestAIOpenScaleClient(unittest.TestCase):
    ...
    def test_06_subscribe(self):
        subscription = TestAIOpenScaleClient.ai_client.data_mart.subscriptions.add(WatsonMachineLearningAsset(
            ...
            feature_columns=["Gender", "Status", "Children", "Est Income", "Car Owner", "Age",
    ...
    def test_10_score(self):
        payload_scoring = {
                "fields": ["Gender", "Status", "Children", "Est Income", "Car Owner", "Age",
                           "LongDistance", "International", "Local", "Dropped", "Paymethod", "LocalBilltype",
                           "LongDistanceBilltype", "Usage", "RatePlan"],
                "values": [
                    ["M", "M", 2.000000, 20078.000000, "N", 32.846667, 9.650000, 0.000000, 6.330000, 0.000000,
    ...
    def test_16_send_feedback_data(self):
        feedback_records = []
        fields = ["CHURN", "Gender", "Status", "Children", "Est Income", "Car Owner", "Age", "LongDistance",
                  "International", "Local", "Dropped",
                  "Paymethod", "LocalBilltype", "LongDistanceBilltype", "Usage", "RatePlan"]

        TestAIOpenScaleClient.feedback_records = 40
        for i in range(0, int(self.feedback_records / 5)):
            feedback_records.append(
                ["T", "F", "S ", 1.000000, 38000.000000, "N", 24.393333, 23.560000, 0.000000, 206.080000, 0.000000,
                 "CC", "Budget", "Intnl_discount", 229.640000, 3.000000])
            feedback_records.append(
                ["F", "F", "M", 1.000000, 57063.000000, "Y", 52.333333, 16.790000, 0.000000, 81.300000, 0.000000, "CH",
                 "Budget", "Standard", 98.100000, 4.000000])
            feedback_records.append(
                ["F", "F", "M", 1.000000, 77129.100000, "Y", 61.846667, 24.770000, 0.000000, 20.070000, 0.000000, "CH",
                 "FreeLocal", "Intnl_discount", 44.850000, 4.000000])
            feedback_records.append(
                ["F", "F", "M", 0.000000, 28770.900000, "N", 12.340000, 27.990000, 5.320000, 16.270000, 0.000000, "CH",
                 "FreeLocal", "Standard", 49.600000, 4.000000])
            feedback_records.append(
                ["T", "F", "S", 2.000000, 15688.500000, "N", 53.060000, 14.390000, 0.000000, 8.030000, 0.000000, "CC",
                 "Budget", "Standard", 22.420000, 4.000000])

        TestAIOpenScaleClient.subscription.feedback_logging.store(fields=fields, feedback_data=feedback_records)
    ...
    def test_19_feedback_logging_data_distribution(self):
        ...
        self.assertGreater(run_result.shape[0], 0)
        self.assertGreater(run_result.shape[1], 0)
        data_columns = run_result.columns.values
        self.assertIn("Car Owner", data_columns)
        self.assertIn("Gender", data_columns)
