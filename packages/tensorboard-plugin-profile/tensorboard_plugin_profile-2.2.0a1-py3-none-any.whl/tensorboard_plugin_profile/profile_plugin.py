...
from tensorflow.python.eager import profiler_client  # pylint: disable=g-direct-tensorflow-import
...
_RAW_DATA_TOOLS = frozenset(
    tool for tool, extension in TOOLS.items() if extension.endswith('.json'))
...
def _parse_filename(filename):
  ...
  if len(parts) >= 2:
    if parts[-1] in ['json', 'pb']:
      parts[-2] = '.'.join(parts[-2:])
      parts.pop()
    else:  # e.g. host.tracetable, host.domain.tracetable
      parts = filename.rsplit('.', 1)
  if len(parts) == 2:  # e.g., host|tracetable, host|op_profile.json
    host, extension = parts[0], parts[1]
  else:  # len(parts) == 1; e.g., trace, tensorflow_stats.pb
    host, extension = None, parts[0]
  return host, _EXTENSION_TO_TOOL.get(extension)
...
def respond(body, content_type, code=200):
...
class ProfilePlugin(base_plugin.TBPlugin):
  ...
  def data_impl(self, request):
    ...
    if tool == 'tensorflow_stats':
      if tqx == 'out:csv;':
        return tf_stats_proto_to_gviz.to_csv(raw_data)
      else:
        return tf_stats_proto_to_gviz.to_json(raw_data)
    if tool == 'overview_page@':
      return overview_page_proto_to_gviz.to_json(raw_data)
    if tool == 'input_pipeline_analyzer@':
      return input_pipeline_proto_to_gviz.to_json(raw_data)
