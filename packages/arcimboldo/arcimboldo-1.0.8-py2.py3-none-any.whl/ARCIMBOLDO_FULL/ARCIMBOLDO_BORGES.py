        ...
        for clusi in range(len(orderedClusters)):
            ...
            if swap_model_after_translation != None:
                ...
                if not os.path.exists(os.path.join(current_directory, "./7_SWAP/" + str(i) + "/")):
                    ...
                    # NOTE this block is just for uncommenting if we need to check the superposition makes sense
            ...
            if topExp_run is not None:
                ...
                if alixe:
                    ...
                    if alixe_mode == 'monomer':
                        tuple_phi, ali_confdict = al.startALIXEforARCIMBOLDO_OneStep(ali_confdict, input_mode,
                                                                                     current_directory,
                                                                                     clust_fold_rotclu,runstring)
                        # NOTE CM: New format tuple_phi = (name.ins, list_files_to_expand)
                        if tuple_phi[1] != []:  # If any phi, call SELSLIB2.shelxe_cycle_BORGES
                            print('\n *Info* ALIXE found consistent phase sets, sending them to expansion')
                            ...
                            #    print("\n There are more ALIXE clusters than topExp, do something", topExp)
                            #    quit()
