...
class UARM:
    ...
    def __init__(self, camera_port="COM4", uarm_port="COM3", serial_baudrate=115200, serial_timeout=1):
        # Setup camera serial
        self.camera = serial.Serial(camera_port, baudrate=serial_baudrate, timeout=serial_timeout)
        # Setup uARM
        self.uarm = SwiftAPI()
        self.uarm.set_position(self.xPos, self.yPos, self.zPos, speed=250)
        self.flush_cmd()
    ...
    def run(self, func):
        ...
        while self.mode == 1 and self.uarm.connected:
            lambda arg: func(arg)
        else:
