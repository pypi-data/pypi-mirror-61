...
# get model metadata
model = requests.get(f'http://localhost:8000/api/models/set/{MODEL_SET_ID}', headers=HEADERS)
print(model.json())
model_id = model.json()["id"]

# get transactions
transactions = requests.get(f'http://localhost:8000/api/transactions/?model={model_id}', headers=HEADERS)
