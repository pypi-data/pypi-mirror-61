...
class GdLogging:
	...
	def __init__(self, LOG_ECS_HOST, service_name, LOG_ECS_INDEX = 'gdlogging', LOG_ECS_TYPE = '_doc', username = None, password = None ):
		try:
			...
			try:
				if self.es.ping():
					print("Pinged to ES Successfully, You're all set.")
	...
	def message_info(self, message, info = None, value = int(1), error=False):
		'''
		gdl.message_info({"message":"sample message","value":bert_count,error=False})
		'''
		try:
			if error:
				log_type="error"
