...
if __name__ == '__main__':
	...
	"""
	This callback will ensure the two images
	are always scaled to the same height:
	"""#:
	@s2.AnimationCallback
	def EqualizeHeights( self, t ):
		self.scaledHeight = s.scaledHeight # automatically computes the `.scaling` factor for the desired target height
		self.scaling = self.scaling[ 1 ]   # ensures isometric scaling (since the line above *only* changes the height)
