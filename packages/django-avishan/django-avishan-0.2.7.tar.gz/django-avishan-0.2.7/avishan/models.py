    ...
    def _clean_model_data_kwargs(cls, on_update: bool = False, **kwargs):
        ...
        for field in cls.get_full_fields():
            ...
            if isinstance(field, (models.OneToOneField, models.ForeignKey)):
                ...
                elif field.related_model == TranslatableChar:
                    if isinstance(kwargs[field.name], dict):
                        base_kwargs[field.name] = TranslatableChar.create(**kwargs[field.name])
                    else:
                        base_kwargs[field.name] = TranslatableChar.create(
                            **{get_avishan_config().LANGUAGE.lower(): kwargs[field.name]}
                        )
                else:
