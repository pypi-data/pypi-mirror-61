...
class ReinforcementLearningRun(Run):
    ...
    """An experiment run class to handle and monitor Reinforcement Learning Runs associated with an
    Experiment and individual run id.
    ...
    .. remarks::
        ...
        :class:`azureml.contrib.train.rl.ReinforcementLearningConfiguration` to see an examnple of this
        process.
    ...
    def __init__(self, experiment, run_id, directory=None, _run_config=None, **kwargs):
        ...
        if _run_config is not None:
            self._run_config_object = ReinforcementLearningConfiguration._get_run_config_object(_run_config)
        else:
    ...
    def complete(self):
        ...
        .. code-block:: python
            ...
            run = experiment.submit(config=ReinfrocementLearningRunConfig)
            run.complete()
