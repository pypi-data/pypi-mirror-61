...
class GraphSession(object):
    ...
    def __token_endpoint(self):
        return self.config['authority_url'] + self.config['tenant_id'] + self.config['token_endpoint']
    ...
    def obtain_token(self):
        ...
        token_response = requests.post(self.__token_endpoint(),
                                       data=data)
    ...
    def obten_data(self, url):
        ...
        while condition:
            ...
            graphdata = self.get(endpoint).json()

            data.extend(graphdata['value'])

            if '@odata.nextLink' not in graphdata: condition = False
