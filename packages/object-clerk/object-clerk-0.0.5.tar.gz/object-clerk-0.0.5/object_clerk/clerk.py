...
class ObjectClerk:
    ...
    @mutate_client_exceptions
    def _list_objects(self, bucket: str, max_keys: int) -> Tuple[List[dict], bool]:
        """
        ...
        return response['Contents'], response['IsTruncated']

    def list_objects(self, bucket: str, max_keys: int = 1000) -> Tuple[List[dict], bool]:
        """
        ...
        response = retry_call(
            self._list_objects,
            fkwargs={"bucket": bucket, "max_keys": max_keys},
            **self.retry_config,
        )
        return response
