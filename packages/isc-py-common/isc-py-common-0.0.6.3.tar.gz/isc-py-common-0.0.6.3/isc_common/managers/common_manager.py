...
class CommonQuerySet(QuerySet):
    ...
    def get_range_rows(self, start=None, end=None, function=None, json=None, *args, **kwargs):
        ...
        if self.alive_only is False:
            if start is not None and end is not None:
                queryResult = super().order_by(*ob).filter(*args, criteria)[start:end]
    ...
    def get_range_rows1(self, request, function=None):
        ...
        return self.get_range_rows(start=request.startRow, end=request.endRow, function=function, json=request.json, alive_only=request.alive_only)
...
class CommonManager(Manager):
    ...
    def get_range_rows1(self, request, function=None):
        request = DSRequest(request=request)
        return self.get_queryset().get_range_rows(start=request.startRow, end=request.endRow, function=function, json=request.json)
    ...
    def updateFromRequest(self, request, removed=None, function=None):
        ...
        if self._dataIsArray(data):
            ...
            if function:
                ...
                if request.get_id():
                    super().filter(id=request.get_id()).update(**self.clone_data(data))
                else:
