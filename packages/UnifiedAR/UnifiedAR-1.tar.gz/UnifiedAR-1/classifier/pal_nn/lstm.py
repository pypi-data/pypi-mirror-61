...
class LSTM_Legacy:
    ...
    def __init__(self, num_features, num_classes, num_units, num_steps, optimizer=None):
        ...
        outputs, states = tf.nn.static_rnn(self.cell, hidden_y, initial_state=self.init_state)
    ...
    def fit(self, x, y, batch_size=100, iter_num=100, summaries_dir=None, summary_interval=10,
        ...
        injector = BatchSequenceInjector(data_x=x, data_y=y, batch_size=batch_size, length=self.num_steps)
        # Train/Test sequence for brief reporting of accuracy and loss
        train_seq_x, train_seq_y = BatchSequenceInjector.to_sequence(self.num_steps, x, y )
    ...
    def predict(self, x, session=None, batch_size=500):
        if session is None:
            if self.sess is None:
                ...
                session = self.sess
        injector = BatchSequenceInjector(batch_size=batch_size, data_x=x, length=self.num_steps)
        injector.reset()
        result = None
        while injector.num_epochs == 0:
            batch_x = injector.next_batch()
    ...
    def __init__(self, num_features, num_classes, num_hidden, num_units, num_skip=0, graph=None, optimizer=None):
        ...
        with self.graph.as_default():
            ...
            with tf.name_scope('rnn'):
                # Apply RNN
                self.cell = tf.nn.rnn_cell.BasicLSTMCell(num_units=num_units, state_is_tuple=True)
