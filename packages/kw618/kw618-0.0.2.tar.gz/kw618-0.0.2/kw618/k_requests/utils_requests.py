...
def record_log(txt, log_name="log"):
    if "log" not in os.listdir():
        os.mkdir("log")
    # # 下面代码应该可以捕获异常
    # tb_txt = traceback.format_exc(limit=3)
    # print(tb_txt)
    with open("./log/{0}.txt".format(log_name), "a", encoding="utf-8") as f:
        f.write("{0}\n{1}\n\n\n".format(time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()), txt))
        ...
        message_html = "sss<br>i love you\naaaa"
...
class kEmailSenderExchange():
    ...
    def __init__(self):
        ...
        self.my_email_address = "{}@{}".format(self.user_name, self.email_domain)
    ...
    def login(self):
        """后续应该建立异常捕获机制"""
        # 连接/登录 我的邮箱
        self.cred = Credentials(r'{}\{}'.format(self.email_domain, self.user_name), self.password)
        self.config = Configuration(server=self.server_address, credentials=self.cred, auth_type=NTLM)
        self.account = Account(
            primary_smtp_address=self.my_email_address, config=self.config, autodiscover=False, access_type=DELEGATE
        )
        print('我的邮箱已连接/登录...\n')
...
class myRequest():
    ...
    def req(self, url, proxies=None, selector=False, auth=None, allow_redirects=True,
        ...
        print("\nreq 已发出。。\n")
        if req_method == "get":
            res_obj = requests.get(url, headers=headers,proxies=proxies, auth=auth, timeout=timeout, allow_redirects=allow_redirects)
        # 下面的 data参数 必须要先用json.dumps()转成str后，才能发送请求！！！
        elif req_method == "post":
            res_obj = requests.post(url, headers=headers,proxies=proxies, data=data, timeout=timeout, allow_redirects=allow_redirects)
        elif req_method == "put":
            res_obj = requests.put(url, headers=headers,proxies=proxies, data=data, timeout=timeout, allow_redirects=allow_redirects)
        ...
        # 5.当需要返回原始response对象的时候
        if is_obj:
            return res_obj
        else:
            res_txt = res_obj.text

        # 6.当需要对页面进行解析的时候可以开启这段代码
        if selector:
            selector = Selector(text=res_txt)
            return selector
    ...
    def robust_req(self, url, proxies=None, selector=False, auth=None,
        ...
        for retry_time in range(1, 3):
            ...
            # 如果response都不在上述判断中,则视为ip被封,更改ip代理,重新req一遍
                ...
                print("\nip被封，更换ip...\n")
                res_txt = self.req(url, proxies="aby", selector=selector, auth=auth,
                                    data=data, selenium=selenium, req_method=req_method,
