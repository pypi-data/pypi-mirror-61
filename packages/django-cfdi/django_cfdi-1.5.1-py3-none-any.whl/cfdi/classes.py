...
class CFDI:
    ...
    def cancelar_prodigia(self):
        ...
        url += f"&keyPass={self.csd_pass}"
        url += f"&arregloUUID={self.cfdi_uuid}"
        url += f"|{self.rfc_receptor}"
        url += f"|{self.rfc}"
        url += f"|{self.total}"
        ...
        txt = requests.post(url, headers=headers, timeout=30)

        try:
            respuesta_json = json.loads(respuesta)
        except Exception as e:
            self.error_conexion = "Hubo un error al intentar parsear el JSON: %s - %s prodigia" % (respuesta, e)
            return [False, self.error_conexion]

        if respuesta_json.get("success"):
            return [True, str(respuesta_json)]

        return [False, str(respuesta_json)]
