...
def get_xml_object(xml_text):
    ...
    for c in conceptos:
        ...
        importe_con_descuento = (
            to_decimal(c.get("importe")) - 
            to_decimal(descuento)
        )
        if xml.es_v33:
            ...
            for tras in c.find_list("traslado"):
                ...
                if not to_decimal(tras.get("base")):
                    continue 
                ...
                descuento_traslado = descuento * factor_traslado
                traslados.append({
                    "impuesto":tras.get("impuesto"),
                    "base":base_traslado,
                    "tasa":tras.get("tasaocuota"),
                    "tipo_ieps":tras.get("tipofactor").lower(),
                    "importe_traspaso":importe_traspaso,
                    "descuento":descuento_traslado
                })
                    ...
                    if tasa_iva:
                        ...
                        if to_decimal(tasa_iva):
                            if es_frontera:
                                ...
                                xml.descuento_tasa_frontera += descuento_traslado 
                                ...
                                xml.descuento_tasa_general += descuento_traslado 
                                if not es_combustible:
                                    xml.importe_tasa_general += importe_tasa

                        elif es_tasa_cero:
                            xml.importe_tasa_cero += importe_tasa
                            xml.descuento_tasa_cero += descuento_traslado
                            xml.total_tasa_cero += (
                                importe_tasa + total_traslado_concepto
                            )

                elif tras.get("impuesto").upper() == "003":
                    if es_combustible and not total_ieps:
                        cuota_ieps = (
