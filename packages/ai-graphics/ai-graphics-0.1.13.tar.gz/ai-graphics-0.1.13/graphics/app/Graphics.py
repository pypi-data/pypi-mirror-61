...
def crop_margin(image, thres=220, margin=[3, 3, 3, 3]):
    """
    计算图片裁剪（白底/透明）边缘大小
    :param image: 输入图像（numpy data）
    :param thres: 阈值
    :param margin: 边缘留白距离（up, down, left, right）
    :return: 输出边框（up, down, left, right）
    """

    width, height = image.shape[:2][::-1]
    if len(image.shape) < 3:
        new = cv2.threshold(image, thres, 255, cv2.THRESH_BINARY_INV)[-1]
        points = np.argwhere(new[:, :] > 0)[:, ::-1]
    elif image.shape[-1] == 3:
        new = cv2.cvtColor(image, cv2.COLOR_RGB2GRAY)
        new = cv2.threshold(new, thres, 255, cv2.THRESH_BINARY_INV)[-1]
        points = np.argwhere(new[:, :] > 0)[:, ::-1]
    else:
    ...
    top, down = max(y - margin[0], 0), min(y + h - 1 + margin[1], height - 1)
    left, right = max(x - margin[2], 0), min(x + w - 1 + margin[3], width - 1)
