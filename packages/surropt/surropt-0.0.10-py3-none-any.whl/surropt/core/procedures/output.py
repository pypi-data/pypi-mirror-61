...
class Report(ABC):
    ...
    def build_iter_report(self, iter_count: int, x: list, f_pred: float,
        ...
        if header:
            ...
            # from https://bugs.python.org/issue24920: "for a GUI app the
            # Windows C runtime leaves the standard FILE streams uninitialized
            # to an invalid file descriptor (-1), so Python's sys.__stdout__
            # is None. That's why you get an AttributeError complaining that
            # NoneType (i.e. type(None)) has no attribute 'fileno'."

            # Attempt at fix for now:
            # try to get default terminal width, resort to 80 chars if it fails
            try:
