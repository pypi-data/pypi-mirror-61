...
class IndexGUI(object):
    ...
    def __init__(self, file_path):
        self.pxpy = np.array(np.loadtxt(file_path, dtype=str, delimiter=','), dtype=float)
        self.source_index = 0
        ...
        # distance label
        o = tk.Label(self.root, text="x pixel:", width=10)
        o.grid(row=2, column=0)

        # x pixel
        self.x = tk.DoubleVar(0)
        l = tk.Entry(self.root, textvariable=self.x)
        l.grid(row=2, column=1)

        o = tk.Label(self.root, text="y pixel:", width=10)
        o.grid(row=2, column=2)

        # y pixel
        self.y = tk.DoubleVar(0)
        l = tk.Entry(self.root, textvariable=self.y)
        l.grid(row=2, column=3)

        # Find idx
        variable = tk.DoubleVar(0)
        o = tk.Button(self.root, text='Find closest spot', command=self.find_closest_spot)
    ...
    def find_closest_spot(self):
        idx = self.closest_spot(self.x.get(), self.y.get(), self.pxpy)

        if idx > -1:
            self.index_var.set(idx)
            self.update_ui(idx)
        else:
            print(f"No spot close to this x,y")

    def closest_spot(self, x, y, spots):
        min = 1000
        res = -1

        # Looking for the closest spot to given x,y
        for i in range(len(spots)):
            dist = (spots[i, 0] - x)**2 + (spots[i, 1] - y)**2
            if dist < min:
                min = dist
                res = i
                print(f"idx: {res}")
