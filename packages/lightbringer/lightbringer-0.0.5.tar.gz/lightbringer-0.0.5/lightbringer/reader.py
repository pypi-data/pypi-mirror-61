...
class SingleMessageReader:
	...
	async def _set_rdy(self, n):
		conns = list(self._reader._direct_connections)
		for l in self._reader._lookups:
			l._conn_args['max_in_flight'] = n
			conns.extend(l._connections.values())
		if conns:
			await asyncio.wait([c._cmd(f'RDY {n}') for c in conns])
