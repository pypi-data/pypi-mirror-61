...
class ClientRunner:
    ...
    def logger(self) -> logging.Logger:
        if not self._log_config:
            return log_utils.NULL_LOGGER

        filename, verbosity, namespace = self._log_config.get_params()
        logger = log_utils.get_logger(namespace=namespace, filename=filename, verbosity=verbosity)
        return logger
    ...
    def time_fit(self, m, X, y, sample_weight=None):
        ...
        """
        with self.activity_logger.log_activity(logger=self.logger,
    ...
    def _get_activity_logger(self) -> ActivityLogger:
        if self._log_config is None:
            ...
            try:
                ...
                from azureml.train.automl._logging._telemetry_activity_logger import TelemetryActivityLogger
                filename, verbosity, namespace = self._log_config.get_params()
                ...
                return cast(ActivityLogger, logger)
            except Exception:
                return DummyActivityLogger()
