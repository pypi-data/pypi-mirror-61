...
def get_contact_from_email(email):
    contacts = sf.query(f"SELECT Id, AccountId, Email FROM Contact WHERE Email='{email}'")['records']
    if len(contacts) != 1:
...
def create_lead(data):
    ...
    """
    allowed_keys = ['Email','FirstName','LastName','Company','LeadSource','Status','OwnerId','Funktion__c',]
    data_keys = data.keys()
...
def update_jwall(df):
    ...
    for row in abacus_pay.iterrows():
        ...
        res = sf.Invoices__c.update(contact['Id'],{
            'Rechnung_bezahlt__c':zlg_datum
        })
        results.append(res)
        print(res)
    return results
...
def jwall_df_from_excel(filename):
    ...
    """
    abacus_pay = pd.read_excel(filename).groupby('Beleg-Nr').agg({
        'Zlg-Datum':'last',
        'Kurzname':'first',
        'Betrag':'sum'
    }).astype('str')
