...
def exercise_storage(path: str):
    ...
    file_stream = force_sync(client.get_file)(bucket_name, "files/file1.txt")

    with open(os.path.join(test_data_path, filename), "rb") as fin:
        test_file_bytes = BytesIO(fin.read())
        assert file_stream.getvalue() == test_file_bytes.getvalue()

    file_stream2 = force_sync(client.file_exists)(bucket_name, "files/file3.txt")
    assert file_stream2 is False

    file_stream2 = force_sync(client.get_file)(bucket_name, "files/file3.txt")

    assert file_stream2 is None
