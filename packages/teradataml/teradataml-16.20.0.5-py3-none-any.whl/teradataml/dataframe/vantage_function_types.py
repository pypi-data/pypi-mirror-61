...
VANTAGE_FUNCTION_ARGTYPE_DEPENDENT_MAPPER = {
    ...
    'DEGREES': lambda x: x if __if_numeric_type(x) else (FLOAT() if __if_categorical_type(x) else None),
    'RADIANS': lambda x: x if __if_numeric_type(x) else (FLOAT() if __if_categorical_type(x) else None),
...
def retrieve_function_expression_type(expression):
    """
    ...
    try:
        ...
        # We are here that mean there is no direct mapping available for the
