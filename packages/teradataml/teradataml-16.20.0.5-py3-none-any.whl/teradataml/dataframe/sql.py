...
class _SQLColumnExpression(_LogicalColumnExpression,
    ...
    def type(self):
        ...
        """
        if isinstance(self.expression.type, sqlalc.sql.sqltypes.NullType):
            if isinstance(self.expression, sqlalc.sql.elements.Over) \
                    or isinstance(self.expression, sqlalc.sql.functions.Function):
                from teradataml.dataframe.vantage_function_types import retrieve_function_expression_type
                return retrieve_function_expression_type(self.expression)
