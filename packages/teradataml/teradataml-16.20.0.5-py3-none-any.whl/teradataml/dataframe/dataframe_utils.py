...
class DataFrameUtils():
    ...
    def _construct_sql_expression_for_aggregations(column_names, column_types, func, percentile=.5, describe_op=False,
        ...
        PARAMETERS :
            ...
            kwargs       -  Keyword argument for time series aggregate functions.
        ...
        is_time_series_aggregate = False
        if "is_time_series_aggregate" in kwargs.keys():
            is_time_series_aggregate = kwargs["is_time_series_aggregate"]

        if is_time_series_aggregate and func in ['bottom', 'bottom with ties', 'delta_t', 'mad', 'top', 'top with ties']:
            return DataFrameUtils._construct_sql_expression_for_time_series_aggregations(column_names, column_types,
                                                                                         func, **kwargs)

        # Check whether distinct values are to be calculated or not.
        distinct = False
        if "distinct" in kwargs.keys():
        ...
        for column in operations:
            ...
            for operation in operations[column]:
                ...
                if not describe_op and type(column_type) in \
                        _Dtypes._get_unsupported_data_types_for_aggregate_operations(operation,
                                                                                     is_time_series_aggregate = is_time_series_aggregate):
                    ...
                    if operation == 'mean':
                        ...
                        td_operation = 'count'
                    elif operation == 'percentile' and not is_time_series_aggregate:
                    ...
                    if operation == 'percentile' and not is_time_series_aggregate:
                        if describe_op:
                        ...
                        quoted_parent_column_name = "{0}, {1}, Linear".format(quoted_parent_column_name, percentile*100)
                    elif operation == 'unique':
                        ...
                        quoted_parent_column_name = "cast({0} as Real)".format(quoted_parent_column_name)

                    if describe_op and type(column_type) in \
                        _Dtypes._get_unsupported_data_types_for_describe_operations(operation):
                        ...
                        if operation == 'percentile' and not is_time_series_aggregate:
                            ...
                            if distinct and operation not in ['mode', 'unique']:
                                quoted_parent_column_name = "{0} {1}".format("DISTINCT", quoted_parent_column_name)
                            valid_columns.append('cast({2}({0}) as Number) as {1}'.format(
                                quoted_parent_column_name, quoted_new_column_name, td_operation))
                    else:
                        if distinct and operation not in ['mode', 'unique']:
                            quoted_parent_column_name = "{0} {1}".format("DISTINCT", quoted_parent_column_name)
                        valid_columns.append('{2}({0}) as {1}'.format(quoted_parent_column_name,
                    ...
                    elif operation in ['mad'] and type(column_type) in [DECIMAL, NUMBER]:
                    ...
                    elif operation in ['mad', 'mean', 'std', 'var'] and type(column_type) in \
                    ...
                    elif operation in ['median'] and is_time_series_aggregate:
                        new_column_types.append(FLOAT())
                    else:
                        new_column_types.append(column_type)
    ...
    @staticmethod
    def _construct_sql_expression_for_time_series_aggregations(column_names, column_types, func, **kwargs):
        """
        Internal function to create and return the sql expression
            ...
            if "default_constant_for_columns" in kwargs.keys():
                ...
                if len(column_names) > 0:
                    aggregate_expr, all_new_column_names, all_new_column_types = \
                        DataFrameUtils._construct_sql_expression_for_aggregations(column_names, column_types, func)
    ...
    @staticmethod
    def _construct_describe_query(table_name, metaexpr, percentiles, function_label, groupby_column_list=None,
        ...
        PARAMETERS :
            table_name:
                Required Argument.
                Specifies table name or top level view name for the dataframe we are collecting statistics for.
                Types: str
            ...
            if op == "percentile":
                for p in percentiles:
                    agg_expr, new_col_names, new_col_types = \
                        DataFrameUtils._construct_sql_expression_for_aggregations(
