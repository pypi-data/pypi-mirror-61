...
class AedUtils:
    ...
    def _aed_show_query_length(self, nodeid, include_all_queries = False, query_with_reference_to_top = False, query_count = None):
        ...
        if not include_all_queries and not query_with_reference_to_top:
            ...
            option = 1
            # Making call to _aed_gen_exec_queries to get number of 
            # queries for nodeid, as it generates queries that
            # actually executes/executed on the database.
            query_count_verify = self._aed_gen_exec_queries(nodeid)
