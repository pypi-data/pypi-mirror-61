...
#def is_exit_node(ip, exits=ExitNodes(*args, **requests_kwargs)):
            ...
            with open(self.cache_path, 'rb') as fp:
                for line in fp:
                    if line.startswith(b'#'):
                        continue
                    if ip.encode('utf-8') in line:
                        logging.debug("{} is a TOR exit node.".format(ip))
                        return True
            return False
        except Exception as e:
            logging.error("Problem occured reading exit nodes: {}".format(e))
            return False
    '''