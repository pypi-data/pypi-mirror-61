...
def download_file_to_path(url, path):
    logging.info('Downloading source from %s to %s' % (url, path))
    ...
    with open(tmp_path, 'wb') as f:
        ...
        if req.status_code != 200:
            logging.error('Downloading source from %s failed' % url)
    ...
    logging.info('%s downloaded to %s' % (url, path))
...
def get_json_by_name_or_path(source: str, source_type: str = 'data_sources'):
    ...
    logging.info('loading %s from: %s' % (source_type, source))
