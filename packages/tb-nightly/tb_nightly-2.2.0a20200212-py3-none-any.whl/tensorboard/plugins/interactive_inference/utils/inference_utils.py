...
class VizParams(object):
    ...
    def __init__(
        self, x_min, x_max, examples, num_mutants, feature_index_pattern
    ):
        """Inits VizParams may raise InvalidUserInputError for bad user
        inputs."""
        ...
        def convert_pattern_to_indices(pattern):
            """Converts a printer-page-style pattern and returns a list of
            indices.
...
class MutantFeatureValue(object):
    ...
    def __init__(self, original_feature, index, mutant_value):
        ...
        self.mutant_value = (
            mutant_value.encode()
            if isinstance(mutant_value, string_types)
            else mutant_value
        )
...
class ServingBundle(object):
    ...
    def __init__(
        ...
        if not isinstance(inference_address, string_types):
            raise ValueError(
                "Invalid inference_address has type: {}".format(
                    type(inference_address)
                )
            )
        # Clean the inference_address so that SmartStub likes it.
        self.inference_address = inference_address.replace(
            "http://", ""
        ).replace("https://", "")

        if not isinstance(model_name, string_types):
            raise ValueError(
                "Invalid model_name has type: {}".format(type(model_name))
            )
...
def proto_value_for_feature(example, feature_name):
    ...
    if feature is None:
        raise ValueError(
            "Feature {} is not on example proto.".format(feature_name)
        )
    feature_type = feature.WhichOneof("kind")
    if feature_type is None:
        raise ValueError(
            "Feature {} on example proto has no declared type.".format(
                feature_name
            )
        )
...
def wrap_inference_results(inference_result_proto):
    ...
    if isinstance(
        ...
        inference_proto.regression_result.CopyFrom(
            inference_result_proto.result
        )
...
def get_numeric_features_to_observed_range(examples):
    ...
    for example in examples:
        for feature_name in get_numeric_feature_names(example):
            ...
            observed_features[feature_name].extend(
...
def make_mutant_features(original_feature, index_to_mutate, viz_params):
    ...
    if original_feature.feature_type == "float_list":
        ...
        mutant_values = (
            np.linspace(int(lower), int(upper), num_mutants)
            .astype(int)
            .tolist()
        )
        ...
        mutant_values = feature_to_samples[original_feature.feature_name][
            "samples"
        ]
        return [
        ...
        raise ValueError(
            "Malformed original feature had type of: "
            + original_feature.feature_type
        )


def make_mutant_tuples(
    ...
    """
    mutant_features = make_mutant_features(
        original_feature, index_to_mutate, viz_params
    )
    mutant_examples = []
    for example_proto in example_protos:
        for mutant_feature in mutant_features:
            ...
            try:
                feature_list = proto_value_for_feature(
                    copied_example, feature_name
                )
...
def mutant_charts_for_feature(
    example_protos, feature_name, serving_bundles, viz_params
):
    ...
    if isinstance(
        ...
        for idx, classification in enumerate(
            ...
            for class_index, classification_class in enumerate(
                ...
                if (
                    len(classification.classes) == 2
                    and classification_class.label == "0"
                ):
                    continue
                ...
                series[key][mutant_val].append(classification_class.score)
        ...
        for key, mutant_values in iteritems(series):
            for value, y_list in iteritems(mutant_values):
                return_series[key].append(
                    {x_label: value, y_label: sum(y_list) / float(len(y_list))}
                )
        ...
        for idx, regression in enumerate(
        ...
        raise NotImplementedError(
            "Only classification and regression implemented."
        )
...
def get_eligible_features(examples, num_mutants):
    ...
    features_dict.update(
        get_categorical_features_to_sampling(examples, num_mutants)
    )
...
def create_sprite_image(examples):
    ...
    with tf.compat.v1.Session():
        keys_to_features = {
            image_feature_name: tf.io.FixedLenFeature(
