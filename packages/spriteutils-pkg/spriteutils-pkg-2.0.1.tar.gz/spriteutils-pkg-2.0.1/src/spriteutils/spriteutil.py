...
class TotalPixelAlreadySet(Exception):
    """
    Error Raise when try to reassign property `total_pixels_count` of
    class `Sprite`
    """
...
class Sprite:
    ...
    def __init__(self, label, x1, y1, x2, y2):
        ...
        self.__surface = \
            self.__num_of_horizontally_pixels * self.__num_of_vertically_pixels
        self.__centroid = ((x1 + x2)//2, (y1 + y2)//2)
        self.__total_pixels_count = None
        self.__density = None
    ...
    @property
    def density(self):
        """float: Represent the ratio of total pixels of sprite over sprite surface"""
        return self.__density

    @property
    def total_pixels_count(self):
        """
        int: Represent the number of non-background-pixels of this sprite

        The setter method will raise:
            TotalPixelAlreadySet -- if the property has already been set
            TypeError -- if the value pass into the setter is not integer
            ValueError -- if the value pass into the setter is one of
                below condition:
                    - Negative value
                    - Greater than the surface property

        The setter method will calculate the density from the provided
            `total_pixels_count` and assign the result to the read-only
            property `density`

        NOTE: The setter method should only be called by SpriteSheet.find_sprites()
        """
        return self.__total_pixels_count

    @total_pixels_count.setter
    def total_pixels_count(self, total_pixels_count):
        if not isinstance(total_pixels_count, int):
            raise TypeError("arg `total_pixels_count` MUST be an integer")
        if self.total_pixels_count:
            raise TotalPixelAlreadySet
        if total_pixels_count <= 0 or total_pixels_count > self.surface:
            raise ValueError

        self.__total_pixels_count = total_pixels_count
...
class SpriteSheet:
    ...
    def find_sprites(self):
        ...
        for label, polar_points in label_map.label_polar_points.items():
            ...
            sprite.total_pixels_count = total_pixels_count
