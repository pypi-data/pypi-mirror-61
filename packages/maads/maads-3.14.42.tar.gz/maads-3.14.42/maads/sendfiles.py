...
def nlpclassify(username,passw,company,email,thefile,iscategory,maads_rest_url,trainingpercentage=75,retrainingdays=0,retraindeploy=0):
    ...
    with open(thefile, 'r', encoding='utf8') as f:
     ...
     for row in reader:
      ...
      # print(len(row))
       if len(row)>2:
           print("Ignored ROW %d: Improperly formatted CSV.  You have too many commas separating your data: %s" % rn+1,rowstr)
           ...
           print("Ignored ROW %d: Improperly formatted CSV: %s" % rn+1,rowstr)
       rn=rn+1
