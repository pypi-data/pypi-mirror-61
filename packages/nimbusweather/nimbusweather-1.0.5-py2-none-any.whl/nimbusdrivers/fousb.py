...
def power_cycle_station(hub, port):
    ...
    busses = usb.busses()
    if not busses:
        raise WeeWxIOError("Power cycle failed: cannot find USB busses")
    device = None
    for bus in busses:
        for dev in bus.devices:
            if dev.deviceClass == usb.CLASS_HUB:
                ...
                if devid == hub:
                    device = dev
    if device is None:
        raise WeeWxIOError("Power cycle failed: cannot find hub %s" % hub)
    handle = device.open()
    try:
...
def _decode(raw, fmt):
    ...
    if isinstance(fmt, dict):
        ...
        if typ == 'ub':
            ...
            raise WeeWxIOError('decode failure: unknown type %s' % typ)
        if scale and result:
...
class FineOffsetUSB(AbstractDevice):
    ...
    def _get_arcint(self):
        for i in range(self.max_tries):
            ...
            raise WeeWxIOError("Unable to read archive interval after %d tries" % self.max_tries)
        ...
        while True:
            try:
                ...
                if nerr > self.max_tries:
                    raise WeeWxIOError("Max retries exceeded while fetching observations")
...
#
# FIXME: to support multiple usb nimbusdrivers, these should be abstracted to a class
...
#==============================================================================
    ...
    def _read_usb_bytes(self, size):
        ...
        if data is None or len(data) < size:
            raise WeeWxIOError('Read from USB failed')
        return list(data)
        ...
        while True:
            try:
                ...
                if fixed_block['read_period'] is None:
                    raise WeeWxIOError('invalid read_period in get_records')
                if fixed_block['data_count'] is None:
                    raise WeeWxIOError('invalid data_count in get_records')
                if since_ts:
                ...
                nerr += 1
                if nerr > self.max_tries:
                    raise WeeWxIOError("Max retries exceeded while fetching records")
                time.sleep(self.wait_before_retry)
    ...
    def sync(self, quality=None, read_period=None):
        ...
        for data, last_ptr, logged in self.live_data(logged_only=(quality>1)):
            ...
            if data['delay'] is None:
                ...
                count += 1
                if count > maxcount:
                    raise WeeWxIOError('repeated invalid delay while synchronising')
                continue
...
#==============================================================================
    ...
    def _write_byte(self, ptr, value):
        ...
        if not self._write_usb(ptr, value):
            raise WeeWxIOError('Write to USB failed')
