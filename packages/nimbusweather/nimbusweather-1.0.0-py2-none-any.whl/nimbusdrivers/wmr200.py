...
class WMR200ProtocolError(WeeWxIOError):
...
class Packet(object):
    ...
    def append_data(self, char):
        ...
        if len(self._pkt_data) == 2 and \
          self._pkt_data[1] > _WMR200_MAX_PACKET_SIZE:
            raise WeeWxIOError('Max packet size exceeded')
    ...
    def packet_process(self):
        ...
        self._record.update({'units': METRIC})
    ...
    def verify_checksum(self):
        ...
        if DEBUG_CHECKSUM and (self.pkt_id % DEBUG_CHECKSUM) == 0:
            raise CRCError('Debug forced checksum error')
    ...
    def timestamp_record(self):
        ...
        try:
            ...
            logerr(msg)
            raise ViolatedPrecondition(msg)
...
class WMR200(AbstractDevice):
    ...
    def __init__(self, **stn_dict):
        ...
        self._sensor_stat = to_bool(stn_dict.get('sensor_status',
                                                                True))
        # Use pc timestamps or weather console timestamps.
        self._use_pc_time = \
            to_bool(stn_dict.get('use_pc_time', True))

        # Use archive data when possible.
        self._erase_archive = \
            to_bool(stn_dict.get('erase_archive', False))
        ...
        self._ignore_checksum = \
                to_bool(stn_dict.get('ignore_checksum', False))
