...
class WMR9x8ProtocolError(WeeWxIOError):
...
class SerialWrapper(object):
    ...
    def openPort(self):
        ...
        logging.debug("wmr9x8: Opened up serial port %s" % self.port)
...
class WMR9x8(AbstractDevice):
        ...
        while True:
            ...
            if buf[0] == 0xFF and buf[1] == 0xFF and buf[2] in wmr9x8_packet_type_size_map:
                ...
                if sent_checksum == calc_checksum:
                    logging.debug("wmr9x8: Received WMR9x8 data packet.")
                    payload = pdata[2:-1]
                    ...
                    logging.debug("wmr9x8: Invalid data packet (%s)." % pdata)
            ...
            # WM-918 packets have no framing
                ...
                calc_checksum = reduce(operator.add, pdata[0:-1]) & 0xFF
                if sent_checksum == calc_checksum:
                    logging.debug("wmr9x8: Received WM-918 data packet.")
                ...
                logging.debug("wmr9x8: Advancing buffer by one for the next potential packet")
                buf.pop(0)
    ...
    def _port_factory(stn_dict):
        ...
        raise UnsupportedFeature(stn_dict['type'])
...
#
# PYTHONPATH=bin python bin/weewx/nimbusdrivers/wmr9x8.py
...
if __name__ == '__main__':
    ...
    if options.gen_packets:
        logging.debug("wmr9x8: Running genLoopPackets()")
