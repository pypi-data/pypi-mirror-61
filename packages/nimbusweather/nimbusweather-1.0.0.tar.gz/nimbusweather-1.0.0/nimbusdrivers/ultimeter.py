...
class Ultimeter(AbstractDevice):
...
class Station(object):
    ...
    def validate_string(self, buf):
        if len(buf) not in [42, 46, 50]:
            raise WeeWxIOError("Unexpected buffer length %d" % len(buf))
        if buf[0:2] != '!!':
            raise WeeWxIOError("Unexpected header bytes '%s'" % buf[0:2])
        return buf
...
#
# PYTHONPATH=bin python bin/weewx/nimbusdrivers/ultimeter.py
