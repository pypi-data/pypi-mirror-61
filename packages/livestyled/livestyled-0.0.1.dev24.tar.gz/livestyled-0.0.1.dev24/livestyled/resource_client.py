...
class LiveStyledResourceClient(LiveStyledAPIClient):
    ...
    def create_user(
        ...
        for key, value in list(payload.items()):
            if value is None:
                payload.pop(key)
        new_user = self._api_post(
            '{}'.format(UserSchema.Meta.create_url),
            payload
        )
        return User(**UserSchema().load(new_user))
    ...
    def update_push_broadcast(
        ...
        return self._update_resource(PushBroadcastSchema, push_broadcast, attributes)
