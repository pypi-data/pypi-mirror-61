...
from useless_lib import is_from_same
...
class Token:
    ...
    def __repr__(self):
        ...
        return "<{}TOKEN -> body: \"{}\" >".format(clarification, self.token)
...
class Tokenizer:
    ...
    def is_finished_single_operator(self, my_op) -> Tuple[str, bool]:
        ...
        for op in self.procedure.config.specific_operators.keys():
            allow_shuffle = self.procedure.config.specific_operators[op].allow_shuffle
            if (allow_shuffle and self.simplify_single_operator(op)[2] == faze_two) or op == faze_one:
                return op, rev
        return "", False
