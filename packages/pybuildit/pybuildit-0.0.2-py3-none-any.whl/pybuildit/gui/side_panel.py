...
class ModeControlFrame(Tk.Frame):
    ...
    def set_speed_limit(self, v):
        if self.buildit.is_open():
            ...
            tryBuildit(lambda: self.buildit.set_velocity_max_limit(builditGuiInfo.targetDevId, from_rpm(self.speedLimit.get())))
            tryBuildit(lambda: self.buildit.set_velocity_min_limit(builditGuiInfo.targetDevId, from_rpm(-self.speedLimit.get())))
    ...
    def updateSpeedSlider(self, servoStatus):
        if (self.isConnected):
            ...
            slide_to= self.speedSlider["to"]
            current_max_limit_vel= self.buildit.get_velocity_max_limit(builditGuiInfo.targetDevId)/100
            current_min_limit_vel= self.buildit.get_velocity_min_limit(builditGuiInfo.targetDevId)/100
            max_limit = max([current_max_limit_vel,abs(current_min_limit_vel)])

            if slide_to != current_max_limit_vel or abs(current_min_limit_vel):
                self.speedSlider["to"] =max_limit
