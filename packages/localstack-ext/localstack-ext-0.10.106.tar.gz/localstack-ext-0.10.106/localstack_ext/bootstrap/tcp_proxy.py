...
class AttrDict(yDcbj):
 def __init__(self,*args,**kwargs):
  yDcbP(AttrDict,self).__init__(*args,**kwargs)
  self.__dict__=self
def tcp_listen(six,addr,port,blk):
 s=socket.socket(AF_INET6 if six else AF_INET,SOCK_STREAM)
 if six and yDcbT(socket,'IPV6_V6ONLY'):
...
class Half(yDcbi):
 def __init__(self,opt,sock,addr,yDcbK):
  self.opt=opt
  self.sock=sock
  self.addr=addr
  self.yDcbK=yDcbK
  self.name='peer' if self.yDcbK else 'client'
  self.queue=[]
  self.dest=yDcbQ
  self.err=yDcbQ
  self.ready=yDcbU
 ...
 while qs:
  ...
  for q in qs:
   if q.post_wait(r,w,e):
    qs.remove(q)
def main():
 opt={'ip6':yDcbU,'bind_port':2223,'addr':'192.168.43.191','port':22}
 server_loop(opt)
