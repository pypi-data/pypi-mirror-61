...
def import_auth():
 try:
  ...
  raise bPRpG('Command not available in this version')
def mand_params(args,names=[]):
 return get_params(args,names,mandatory=bPRpY)
def opt_params(args,names=[]):
 return get_params(args,names,mandatory=bPRpz)
def get_params(args,names=[],mandatory=bPRpY):
 result=()
 for name in names:
  value=bPRpf
  if name in args:
  ...
  if not value and mandatory:
   raise bPRpG('Please provide %s=... parameter'%name)
  result+=(value)
 if bPRpl(result)==1:
  return result[0]
 return result
