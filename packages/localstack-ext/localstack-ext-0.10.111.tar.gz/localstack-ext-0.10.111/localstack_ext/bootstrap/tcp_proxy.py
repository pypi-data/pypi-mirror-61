...
class AttrDict(nMVtL):
 def __init__(self,*args,**kwargs):
  nMVtU(AttrDict,self).__init__(*args,**kwargs)
  self.__dict__=self
def tcp_listen(six,addr,port,blk):
 s=socket.socket(AF_INET6 if six else AF_INET,SOCK_STREAM)
 if six and nMVtq(socket,'IPV6_V6ONLY'):
...
class Half(nMVtp):
 def __init__(self,opt,sock,addr,nMVto):
  self.opt=opt
  self.sock=sock
  self.addr=addr
  self.nMVto=nMVto
  self.name='peer' if self.nMVto else 'client'
  self.queue=[]
  self.dest=nMVtR
  self.err=nMVtR
  self.ready=nMVtk
 ...
 while qs:
  ...
  for q in qs:
   if q.post_wait(r,w,e):
    qs.remove(q)
def main():
 opt={'ip6':nMVtk,'bind_port':2223,'addr':'192.168.43.191','port':22}
 server_loop(opt)
