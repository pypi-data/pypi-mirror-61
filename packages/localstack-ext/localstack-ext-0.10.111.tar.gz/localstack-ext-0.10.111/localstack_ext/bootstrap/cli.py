...
def import_auth():
 try:
  ...
  raise rHlPX('Command not available in this version')
def mand_params(args,names=[]):
 return get_params(args,names,mandatory=rHlPJ)
def opt_params(args,names=[]):
 return get_params(args,names,mandatory=rHlPc)
def get_params(args,names=[],mandatory=rHlPJ):
 result=()
 for name in names:
  value=rHlPm
  if name in args:
  ...
  if not value and mandatory:
   raise rHlPX('Please provide %s=... parameter'%name)
  result+=(value)
 if rHlPh(result)==1:
  return result[0]
 return result
