...
def import_auth():
 try:
  ...
  raise rRcIN('Command not available in this version')
def mand_params(args,names=[]):
 return get_params(args,names,mandatory=rRcIP)
def opt_params(args,names=[]):
 return get_params(args,names,mandatory=rRcIQ)
def get_params(args,names=[],mandatory=rRcIP):
 result=()
 for name in names:
  value=rRcIb
  if name in args:
  ...
  if not value and mandatory:
   raise rRcIN('Please provide %s=... parameter'%name)
  result+=(value)
 if rRcIF(result)==1:
  return result[0]
 return result
