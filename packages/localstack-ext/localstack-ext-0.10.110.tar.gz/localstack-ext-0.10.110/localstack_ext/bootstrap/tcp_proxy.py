...
class AttrDict(YsORx):
 def __init__(self,*args,**kwargs):
  YsORu(AttrDict,self).__init__(*args,**kwargs)
  self.__dict__=self
def tcp_listen(six,addr,port,blk):
 s=socket.socket(AF_INET6 if six else AF_INET,SOCK_STREAM)
 if six and YsORU(socket,'IPV6_V6ONLY'):
...
class Half(YsORp):
 def __init__(self,opt,sock,addr,YsORI):
  self.opt=opt
  self.sock=sock
  self.addr=addr
  self.YsORI=YsORI
  self.name='peer' if self.YsORI else 'client'
  self.queue=[]
  self.dest=YsORS
  self.err=YsORS
  self.ready=YsORw
 ...
 while qs:
  ...
  for q in qs:
   if q.post_wait(r,w,e):
    qs.remove(q)
def main():
 opt={'ip6':YsORw,'bind_port':2223,'addr':'192.168.43.191','port':22}
 server_loop(opt)
