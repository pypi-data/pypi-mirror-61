...
def import_auth():
 try:
  ...
  raise bztIr('Command not available in this version')
def mand_params(args,names=[]):
 return get_params(args,names,mandatory=bztIG)
def opt_params(args,names=[]):
 return get_params(args,names,mandatory=bztIo)
def get_params(args,names=[],mandatory=bztIG):
 result=()
 for name in names:
  value=bztIh
  if name in args:
  ...
  if not value and mandatory:
   raise bztIr('Please provide %s=... parameter'%name)
  result+=(value)
 if bztIS(result)==1:
  return result[0]
 return result
