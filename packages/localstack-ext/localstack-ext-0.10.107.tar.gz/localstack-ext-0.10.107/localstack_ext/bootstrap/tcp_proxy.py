...
class AttrDict(vHAWY):
 def __init__(self,*args,**kwargs):
  vHAWu(AttrDict,self).__init__(*args,**kwargs)
  self.__dict__=self
def tcp_listen(six,addr,port,blk):
 s=socket.socket(AF_INET6 if six else AF_INET,SOCK_STREAM)
 if six and vHAWh(socket,'IPV6_V6ONLY'):
...
class Half(vHAWt):
 def __init__(self,opt,sock,addr,vHAWb):
  self.opt=opt
  self.sock=sock
  self.addr=addr
  self.vHAWb=vHAWb
  self.name='peer' if self.vHAWb else 'client'
  self.queue=[]
  self.dest=vHAWS
  self.err=vHAWS
  self.ready=vHAWQ
 ...
 while qs:
  ...
  for q in qs:
   if q.post_wait(r,w,e):
    qs.remove(q)
def main():
 opt={'ip6':vHAWQ,'bind_port':2223,'addr':'192.168.43.191','port':22}
 server_loop(opt)
