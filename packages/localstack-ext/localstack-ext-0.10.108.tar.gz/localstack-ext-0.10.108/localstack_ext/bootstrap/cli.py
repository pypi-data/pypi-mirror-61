...
def import_auth():
 try:
  ...
  raise RklIN('Command not available in this version')
def mand_params(args,names=[]):
 return get_params(args,names,mandatory=RklIV)
def opt_params(args,names=[]):
 return get_params(args,names,mandatory=RklIp)
def get_params(args,names=[],mandatory=RklIV):
 result=()
 for name in names:
  value=RklIc
  if name in args:
  ...
  if not value and mandatory:
   raise RklIN('Please provide %s=... parameter'%name)
  result+=(value)
 if RklIe(result)==1:
  return result[0]
 return result
