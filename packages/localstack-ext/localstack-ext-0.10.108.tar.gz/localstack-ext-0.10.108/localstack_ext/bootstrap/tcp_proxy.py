...
class AttrDict(DvBpH):
 def __init__(self,*args,**kwargs):
  DvBpc(AttrDict,self).__init__(*args,**kwargs)
  self.__dict__=self
def tcp_listen(six,addr,port,blk):
 s=socket.socket(AF_INET6 if six else AF_INET,SOCK_STREAM)
 if six and DvBpR(socket,'IPV6_V6ONLY'):
...
class Half(DvBpb):
 def __init__(self,opt,sock,addr,DvBpz):
  self.opt=opt
  self.sock=sock
  self.addr=addr
  self.DvBpz=DvBpz
  self.name='peer' if self.DvBpz else 'client'
  self.queue=[]
  self.dest=DvBpC
  self.err=DvBpC
  self.ready=DvBpm
 ...
 while qs:
  ...
  for q in qs:
   if q.post_wait(r,w,e):
    qs.remove(q)
def main():
 opt={'ip6':DvBpm,'bind_port':2223,'addr':'192.168.43.191','port':22}
 server_loop(opt)
