...
def import_auth():
 try:
  ...
  raise lTHKp('Command not available in this version')
def mand_params(args,names=[]):
 return get_params(args,names,mandatory=lTHKg)
def opt_params(args,names=[]):
 return get_params(args,names,mandatory=lTHKa)
def get_params(args,names=[],mandatory=lTHKg):
 result=()
 for name in names:
  value=lTHKk
  if name in args:
  ...
  if not value and mandatory:
   raise lTHKp('Please provide %s=... parameter'%name)
  result+=(value)
 if lTHKe(result)==1:
  return result[0]
 return result
