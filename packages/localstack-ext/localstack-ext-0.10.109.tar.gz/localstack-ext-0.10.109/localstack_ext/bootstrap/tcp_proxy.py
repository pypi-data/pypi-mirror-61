...
class AttrDict(kNcMi):
 def __init__(self,*args,**kwargs):
  kNcMq(AttrDict,self).__init__(*args,**kwargs)
  self.__dict__=self
def tcp_listen(six,addr,port,blk):
 s=socket.socket(AF_INET6 if six else AF_INET,SOCK_STREAM)
 if six and kNcMf(socket,'IPV6_V6ONLY'):
...
class Half(kNcMu):
 def __init__(self,opt,sock,addr,kNcME):
  self.opt=opt
  self.sock=sock
  self.addr=addr
  self.kNcME=kNcME
  self.name='peer' if self.kNcME else 'client'
  self.queue=[]
  self.dest=kNcMD
  self.err=kNcMD
  self.ready=kNcMG
 ...
 while qs:
  ...
  for q in qs:
   if q.post_wait(r,w,e):
    qs.remove(q)
def main():
 opt={'ip6':kNcMG,'bind_port':2223,'addr':'192.168.43.191','port':22}
 server_loop(opt)
