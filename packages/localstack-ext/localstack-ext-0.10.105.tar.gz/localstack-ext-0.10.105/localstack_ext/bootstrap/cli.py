...
def import_auth():
 try:
  ...
  raise KECxY('Command not available in this version')
def mand_params(args,names=[]):
 return get_params(args,names,mandatory=KECxe)
def opt_params(args,names=[]):
 return get_params(args,names,mandatory=KECxP)
def get_params(args,names=[],mandatory=KECxe):
 result=()
 for name in names:
  value=KECxD
  if name in args:
  ...
  if not value and mandatory:
   raise KECxY('Please provide %s=... parameter'%name)
  result+=(value)
 if KECxa(result)==1:
  return result[0]
 return result
