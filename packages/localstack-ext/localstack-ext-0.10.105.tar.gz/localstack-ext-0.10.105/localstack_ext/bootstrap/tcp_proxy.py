...
class AttrDict(FpUbi):
 def __init__(self,*args,**kwargs):
  FpUbK(AttrDict,self).__init__(*args,**kwargs)
  self.__dict__=self
def tcp_listen(six,addr,port,blk):
 s=socket.socket(AF_INET6 if six else AF_INET,SOCK_STREAM)
 if six and FpUbR(socket,'IPV6_V6ONLY'):
...
class Half(FpUbW):
 def __init__(self,opt,sock,addr,FpUbx):
  self.opt=opt
  self.sock=sock
  self.addr=addr
  self.FpUbx=FpUbx
  self.name='peer' if self.FpUbx else 'client'
  self.queue=[]
  self.dest=FpUbh
  self.err=FpUbh
  self.ready=FpUbV
 ...
 while qs:
  ...
  for q in qs:
   if q.post_wait(r,w,e):
    qs.remove(q)
def main():
 opt={'ip6':FpUbV,'bind_port':2223,'addr':'192.168.43.191','port':22}
 server_loop(opt)
