...
class WorkQueueMapper(AbstractWorkQueueMapper):
    ...
    select_next_sql = "SELECT * FROM work_queue WHERE status='waiting' ORDER BY task_id LIMIT 1"
    update_next_sql = "UPDATE work_queue SET status='processing', actor_id=? WHERE task_id=?"
