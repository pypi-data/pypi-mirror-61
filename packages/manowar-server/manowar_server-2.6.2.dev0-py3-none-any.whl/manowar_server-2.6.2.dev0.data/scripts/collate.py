...
if __name__ == "__main__":
    ...
    parser.add_argument("-c", "--config", help="Main collate.ini file", required=False, default=False)
...
def collate(CONFIG):
    ...
    def grab_single_collated(db_config, result_enum, type_to_grab):
        ...
        grab_single_collated_query_list.append("and last_audit >= FROM_UNIXTIME(%s) ")
        ...
        try:
            ...
            logger.error("Unable to Grab GSCQ Error for Group : {} on type {}".format(grouper, type_to_grab))
    ...
    def grab_all_table_data(db_config):
        ...
        for table in ["acoll", "pop", "srvtype"]:
            ...
            logger.debug("For table {} I:{} U:{}".format(table, this_inserts, this_updates))
    ...
    def compare_update_insert_table(collected, currently_on_disk, table, db_config):
        ...
        try:
            if len(update_ids) > 0 :
                ...
                try:
                    update_query_debug = cur.mogrify(update_query, update_query_parameters)

                    logger.info("Running Update Query for table : {}".format(table))
                    ...
                    logger.error("Error updating hosts for {} : {}".format(table, update_query_error))
                else:
        ...
        try:
            if len(insert_list) > 0:
                ...
                insert_query.append("INSERT into audits_by_{0} ( {0}_text, ".format(table)) # nosec
                insert_query.append("fk_audits_id, {0}_initial_audit, {0}_last_audit, {0}_passed, {0}_failed, {0}_exempt ) ".format(table))
                ...
                try:
                    ...
                    logger.error("Error doing Inserts for {} : {}".format(table, insert_error))
                else:
    ...
    def get_current_table_data(db_config, table):
        ...
        grab_current_table_list.append("SELECT {0}_id, {0}_text, fk_audits_id, {0}_passed, {0}_failed, {0}_exempt ".format(table))
        grab_current_table_list.append("FROM audits_by_{0}".format(table))
        grab_current_table_list.append("WHERE {0}_last_audit >= now() - INTERVAL %s SECOND".format(table))
        grab_current_table_list.append("group by {0}_text, fk_audits_id".format(table))
        ...
        try:
            ...
            logger.error("Unable to grab current table list table {}".format(table))
        else:
            if cur.rowcount:
                    query_results_list = cur.fetchall()
            else :
    ...
    results_dict["updates"], results_dict["inserts"] = grab_all_table_data(config_items)
