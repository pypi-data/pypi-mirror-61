...
class AppHealthCheckMonitor:
    ...
    def _start(self, identifier: AppInstanceIdentifier):
        ...
        if succeed:
            # todo ingress: check if the second condition (app.has_public_access) is deprecated
            check_ingress = self._app_service.should_run_ingress_health_check(identifier=identifier) and \
                            any([app for app in self._apps if app.name == identifier.name and app.has_public_access])
            if check_ingress:
                self._test_ingress_access(identifier=identifier)
            else:
    ...
    def _test_ingress_access(self, identifier: AppInstanceIdentifier):
