...
class OpenEphysNWB(OpenEphysBase):
    ...
    def load(self, pname_root: str, session_name=None, recording_name=None, loadraw=False, loadspikes=False, savedat=False, loadaccelerometer=False):
        '''
        ...
        try:
            ...
            print("No xy data present in nwb file")
        try:
            # TTL data...
        ...
        try:
            ...
            if (loadraw == True):
                ...
                nchans = len(self.settings.channel_info)
                if loadaccelerometer:
                    self.accelerometerData = self.rawData[:,nchans:]
                self.rawData = self.rawData[:,0:nchans]
                if (savedat == True):
                    data2save = self.rawData[:,0:nchans]
