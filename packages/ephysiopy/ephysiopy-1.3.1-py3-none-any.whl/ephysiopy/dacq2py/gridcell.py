...
#        # reverse y,x order
	...
	def getMeasures(self, A, maxima='centroid', field_extent_method=2, allProps=True, **kwargs):
		...
		if maxima == 'centroid':
			...
			except ValueError as e: #might fail as regionprops might not get enough regions i think
				xyCoordPeaks = np.fliplr(np.rot90(np.array(np.nonzero(peaksLabel)))) #  sensible fallback?
		elif maxima == 'single':
