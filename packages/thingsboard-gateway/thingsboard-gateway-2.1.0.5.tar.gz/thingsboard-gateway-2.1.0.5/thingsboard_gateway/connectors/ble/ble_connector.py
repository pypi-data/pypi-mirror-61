...
class BLEConnector(Connector, Thread):
    def __init__(self, gateway, config, connector_type):
        ...
        self.__service_rpc = {
            "ble_scan": self.__scan_ble,
            "ble_connect": self.__check_and_reconnect,
    ...
    def __scan_ble(self, timeout=None, passive_mode=None):
        if not self.__scanning:
            log.info("Scanning for devices...")
            self.__scanning = True
            timeout = timeout if timeout is not None else self.__config.get('scanTimeSeconds', 5)
            passive_mode = passive_mode if passive_mode is not None else self.__config.get('passiveScanMode', False)
            try:
                self.__scanner.scan(timeout, passive=passive_mode)
            ...
            log.debug("Already in scan mode")
...
class ScanDelegate(DefaultDelegate):
    ...
    def handleDiscovery(self, dev, is_new_device, is_new_data):
        if is_new_device:
            ...
            self.__connector._scanned_devices.append(dev)
            log.debug(dev.getValueText(22))
            log.debug(dev.getValueText(9))
            try:
                p = Peripheral(dev)
                if dev.connectable:
                    name = p.getCharacteristics("0x2A00").read()
                    log.debug(name)
            except Exception as e:
                log.exception(e)
        if is_new_data:
            log.debug(dev.getScanData())
            log.debug(dev.getValueText(8))
            log.debug(dev.getValueText(9))
