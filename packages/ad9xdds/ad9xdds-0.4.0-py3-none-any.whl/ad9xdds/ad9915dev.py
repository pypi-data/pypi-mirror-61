...
1.   Remove the comments (first two lines) from the top of converted_2.ihx.
2.   Use objcopy (or any other hex/bin converter) to convert from ihex to binary:
...
AD_VENDOR_ID = 0x0456
AD9914_PRODUCT_ID = 0xee1e
AD9914_PRODUCT_ID = 0xee1f
...
PRODUCT_ID = AD9914_PRODUCT_ID

IFMIN_9915 = 500000000
IFMAX_9915 = 2500000000          # Input maximum frequency (Hz)
IFMIN_9914 = 500000000
IFMAX_9914 = 3500000000          # Input maximum frequency (Hz)
##OFMAX = 1000000000          # Output maximum frequency (Hz)
AMAX = (1<<DAC_OUT_SIZE)-1  # Output maximum amplitude (a.u.)


#==============================================================================
class Ad9915Dev(object):
    """Class for AD9915 development board.
    Board communicates with PC through the USB port of DDS development board.
    """
    ...
    _IN_DAT_EP_ID  = 3       # EP6_IN @0x86 = 134 -> large endpoint
    _OUT_DAT_EP_ID = 2       # EP2_OUT @0x02 = 2  -> large endpoint
    ...
    def __init__(self):
        ...
        """
        super().__init__()
        self._fx2 = iopy.fx2.Fx2Core()
        self._fx2.set_dev(VENDOR_ID, PRODUCT_ID)
        ...
        """self.config(config_id=self._CONFIG_ID, \
                    interface_id=self._INTERFACE_ID, \
                    alt_interface_id=self._ALT_INTF_ID, \
                    in_ep_id=self._IN_EP_ID, \
                    out_ep_id=self._OUT_EP_ID, \
    ...
    def config(self, config_id, interface_id, alt_interface_id, out_ep_id, \
        ...
        try:
            ...
            logging.critical("USB error: %s", str(ex))
            raise
        except Exception as ex:
            logging.critical("Could not set interface configuration: %s", \
                             str(ex))
            raise
        self._dev.reset()
        # Set interface
        try:
            ...
            logging.critical("Could not set alternate interface setting: %s", \
                             str(ex))
            raise
        ...
        try:
            ...
            logging.critical("Could not set endpoint setting: %s", str(ex))
    ...
    def get_reg(self, address):
        ...
        bword = self._build_instruct(1, 0, int(address))
        bword = (bin(bword)).replace('0b', '')
        bword = list(bword)
        bword.reverse()
...
if __name__ == '__main__':
    ...
    start_ = array('B', (0x0F, 0x04, 0x00, 0x00))
    io_update_on_0 = array('B', (0x03, 0x01, 0x08))
    io_update_off_0 = array('B', (0x03, 0x01, 0x00))

    init_cfr2 = array('B', (0x0D, 0x01, 0x00, 0x80, 0x09, 0x00))
    reset_cfr2 = array('B', (0x0D, 0x01, 0x00, 0x00, 0x09, 0x00))

    f_9_0 = array('B', (0x0D, 0x0B, 0x02, 0x4D, 0xD2, 0xF1))
    amp_0 = array('B', (0x0D, 0x0C, 0x00, 0x06, 0x00, 0x00))
    f_11_0 = array('B', (0x0D, 0x0B, 0x02, 0xD0, 0xE5, 0x60))

    DDS = Ad9915Dev()

    # Init Pin Control lines
    print(DDS._fx2.write_ctrl(io_update_off_0))
    print(DDS._fx2.write_ctrl(array('B', (0x03, 0x00, 0x02))))
    print(DDS._fx2.write_ctrl(array('B', (0x03, 0x04, 0x00))))

    # Reset CFR2 register
    print(DDS._fx2.write_ctrl(start_))
    print(DDS._fx2.write_ctrl(reset_cfr2))
    print(DDS._fx2.write_ctrl(io_update_on_0))
    print(DDS._fx2.write_ctrl(io_update_off_0))
    ...
    # Init CFR2 register
    print(DDS._fx2.write_ctrl(start_))
    print(DDS._fx2.write_ctrl(init_cfr2))
    print(DDS._fx2.write_ctrl(io_update_on_0))
    print(DDS._fx2.write_ctrl(io_update_off_0))

    # Read CFR1 register
    print(DDS._fx2.write_ctrl(start_))
    print(DDS._fx2.write_ctrl(array('B', (0x0E, 0x04, 0x80))))
    print(DDS._fx2.read_ctrl())

    # Set amplitude and frequency P0
    print(DDS._fx2.write_ctrl(start_))
    print(DDS._fx2.write_ctrl(amp_0))
    print(DDS._fx2.write_ctrl(start_))
    print(DDS._fx2.write_ctrl(f_9_0))
    print(DDS._fx2.write_ctrl(io_update_on_0))
    print(DDS._fx2.write_ctrl(io_update_off_0))
    ...
    # Set frequency P0
    print(DDS._fx2.write_ctrl(start_))
    print(DDS._fx2.write_ctrl(f_11_0))
    print(DDS._fx2.write_ctrl(io_update_on_0))
    print(DDS._fx2.write_ctrl(io_update_off_0))
