...
class Database:
    ...
    def __init__(self, protocol = 'default_od'):
        ...
        db_json =  root.joinpath('drishtigs1_db_'+self.protocol+'.json')
        # initialize filelist
    ...
    def _make_sample(self, img_path,gt_path):
        ...
        # Each img has a different resolution, so img.size has to be called every time
        gt = GroundTruth(gt_path, drawsize=img.size)
