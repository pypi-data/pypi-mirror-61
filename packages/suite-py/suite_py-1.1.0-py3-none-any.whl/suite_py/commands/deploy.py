...
def get_release(repo):
    ...
    logger.info("L'ultimo tag trovato è {tag}")
    if not prompt_utils.ask_confirm("Sicuro di voler continuare?"):
        sys.exit(-1)
    return repo.create_git_release(tag, tag, "New release from tag {tag}")


def create_release(repo, new_version, message, project, deployed_cards_link):
    new_release = repo.create_git_release(
        new_version, new_version, youtrack.replace_card_names_with_md_links(
            message)
    )
    if new_release:
