...
def select_pr(project):
    ...
    if hasattr(prs, 'totalCount') and prs.totalCount > 0:
        choices = [{"name": "{} {}".format(
            i.number, i.title), "value": i} for i in prs]
        return prompt_utils.ask_choices('Seleziona PR: ', choices)
    else:
        logger.error(
            "Non esistono pull request pronte per il merge o potresti non avere i permessi, per favore controlla su https://github.com/primait/{}/pulls".format(project))
        sys.exit(-1)
