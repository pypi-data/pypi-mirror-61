...
def test_surfaces():
    ...
    count = 0
    res = lasso_type(coordinates = full_array, loop_indices = [1,30])
    count += verify(res, good_out1)

    res = lasso_type(coordinates = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                     '../data/lasso_00.xyz'), loop_indices = [1,30])
    count += verify(res, good_out2)

    res = lasso_type(coordinates = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                     '../data/L1L1.xyz'), loop_indices = [3,6], min_dist=[0,0,0])
    count += verify(res, good_out3)

    res = lasso_type(coordinates = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                     '../data/L1L1.xyz'), loop_indices = [[3,6],[7,10]], min_dist = [0,0,0])
