...
class Invariant(Graph):
    ...
    def calculate(self, invariant, closure=Closure.TWO_POINTS, tries=200, direction=0, reduce_method=ReduceMethod.KMT,
                  poly_reduce=True, translate=False, external_dictionary='', chiral=False, boundaries=[], hide_trivial=True, max_cross=15, matrix=True, density=1, level=0,
                  cuda=True, matrix_plot=False, plot_ofile="KnotFingerPrintMap", plot_format=PlotFormat.PNG,
                  output_file='', output_format=OutputFormat.Dictionary, run_parallel=False, parallel_workers=None, debug=False):
        ...
        if not matrix and len(boundaries) < 2 and matrix_result:  # single chain calculation and non-empty result
