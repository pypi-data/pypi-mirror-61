...
class Reader:
    ...
    def read_stringdictionary_input(self, data):
        ...
        for line in lines:
            ...
            ident = [int(x) for x in ident.strip("'()").split(', ')]
            indices += ident
    ...
    def read_dictionary_input(self, data):
        ...
        for ident in data.keys():
            ...
            knots = data[ident].keys()
            for knot in knots:
                if knot == '0_1' or data[ident][knot] < self.cutoff:
                    continue
                if knot not in self.data.keys():
                    self.data[knot] = []
                self.data[knot].append((ident[0], ident[1], data[ident][knot]))
        return indices
