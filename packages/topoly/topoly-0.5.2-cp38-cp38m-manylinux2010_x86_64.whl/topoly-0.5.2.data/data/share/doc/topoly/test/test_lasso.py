...
good_out1 = '0 4 1 X X -33 +34 -35 -43 XX 0 2 1 X X -33 -43 XX LS2--C XX 10.4002 30 2.42151\n'
good_out2 = '0 0 1 X X XX 0 0 1 X X XX  L0 XX 14.3203 30 2.32774\n'
good_out3 = '1 0 1 X +1 X XX 1 0 1 X +1 X XX L+1N XX 3 7.23607 2.33928\n'
good_out4 = 'loop 3-6: 1 0 1 X +1 X XX 1 0 1 X +1 X XX L+1N XX 3 7.23607 2.33928\nloop 7-10: 0 1 1 X X +11 XX 0 1 1 X X +11 XX L+1C XX 3 7.23607 2.33928\n'

def verify(res, good_out):
    print("result:\n" + res.rstrip('\n'))
    try:
        assert res == good_out
    except:
...
# @pytest.mark.skip
def test_lasso():
    print("Testing PY lassos\n")
    count = 0
    res = lasso_type(full_array, loop_indices=[1, 30])
    count += verify(res, good_out1)

    res = lasso_type('data/lasso_00.xyz', loop_indices=[1, 30])
    count += verify(res, good_out2)

    res = lasso_type('data/L1L1.xyz', loop_indices=[3, 6], min_dist=[0, 0, 0])
    count += verify(res, good_out3)

    res = lasso_type('data/L1L1.xyz', loop_indices=[[3, 6], [7, 10]], min_dist=[0, 0, 0])
    count += verify(res, good_out4)
    print("Passed {}/4".format(count))
