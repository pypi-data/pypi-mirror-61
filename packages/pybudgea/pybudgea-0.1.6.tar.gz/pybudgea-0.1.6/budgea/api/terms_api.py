...
class TermsApi(object):
    ...
    def terms_id_term_post(self, id_term, **kwargs):  # noqa: E501
        ...
        """
        kwargs['_return_http_data_only'] = True
        return self.terms_id_term_post_with_http_info(id_term, **kwargs)  # noqa: E501

    def terms_id_term_post_with_http_info(self, id_term, **kwargs):  # noqa: E501
        """Add terms content for a language. Only for the active terms, there is no use to update obsolete terms  # noqa: E501

        This method makes a synchronous HTTP request by default. To make an
        asynchronous HTTP request, please pass async_req=True
        >>> thread = api.terms_id_term_post_with_http_info(id_term, async_req=True)
        >>> result = thread.get()
        ...
        for key, val in six.iteritems(local_var_params['kwargs']):
            if key not in all_params:
                raise ApiTypeError(
                    "Got an unexpected keyword argument '%s'"
                    " to method terms_id_term_post" % key
                )
            local_var_params[key] = val
        del local_var_params['kwargs']
        # verify the required parameter 'id_term' is set
        if ('id_term' not in local_var_params or
                local_var_params['id_term'] is None):
            raise ApiValueError("Missing the required parameter `id_term` when calling `terms_id_term_post`")  # noqa: E501
