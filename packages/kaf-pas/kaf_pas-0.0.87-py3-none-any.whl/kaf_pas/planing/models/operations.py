...
class OperationsManager(AuditManager):
    ...
    def make_routing(data):
        ...
        sql_str = f'''with recursive r as (
                                            ...
                                            where lower(r1.section) != 'документация'  or r1.parent_id is null
        ...
        with managed_progress(
                ...
                qty=count * 2,
                user=user,
                message=f'<h3>Расчет маршрутизации для: Запуска № {launch.code} от {DateToStr(launch.date)}</h3>',
                title='Выполнено'
        ) as progress:
            ...
            with transaction.atomic():
                with connection.cursor() as cursor:
                    ...
                    #         qty=count * 2,
                    #         user=user,
                    #         message=f'<h3>Инвертирование уровней</h3>',
                    #         title='Выполнено'
                    # ) as progress:
                    #     cursor.execute(f'select dictinct * level from {mat_view_name}')
                    #     rows = cursor.fetchall()
            ...
            progress.sendMessage(type='refresh_grid', message='')
