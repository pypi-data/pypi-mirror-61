...
def relu_activation(Z):
    ...
    print(f'line 50 A.shape: {Z.shape}')
    A = np.maximum(0,Z)
    print(f'line 52 A.shape: {A.shape}')
    cache = Z
    return A, cache
...
def test_function(A_prev):
    print(f'Test line printing A_prev.shape: {A_prev.shape}')
...
def forward_pooling(A_prev, hparameters, mode = "max"):
    ...
    print(f'Line 216 A.shape: {A.shape}')
...
def forward_propogation(X, layers):
    m = X.shape[0]
    A = X
    print(f'Line 217 A.shape: {A.shape}')
    flattened = False
    ...
    for layer in layers[:-1]:
        ...
        #convolution layer
        if layer['mode'] == 'conv':
            print(f'number of times in test: {i}')
            test_function(A)
            if(i==1):
                print(A)
            i+=1
        ...
        #pooling layer
            ...
            print(f'line 245 A.shape is: {A.shape}')

        #fully connected layer
        elif layer['mode'] == 'fc':
            if not flattened:
                flattened_A = (A.reshape(m,-1)).T # flatten
                print(f'the shape of flattend_A is: {flattened_A.shape}')
        ...
        print(f'A shape after {layer["mode"]} is')
        print(f' {A.shape}')
...
def train(X_train, Y_train, X_test, Y_test, layers, num_exp=0, batch_size=10, num_epoch=1, learning_rate=0.01):
    ...
    for epoch in range(num_epoch):
        ...
        for iteration in range(num_iteration):
            ...
            print(f'Line 565 X_train.shape: {X_train.shape}')

            X_train_batch = X_train[index[batch_start:batch_end]]
            Y_train_batch = Y_train[index[batch_start:batch_end]]

            print(f'Line 570 X_train_batch.shape: {X_train_batch.shape}')
