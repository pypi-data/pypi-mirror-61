...
def backward_propagation(AL, Y, layers):
    ...
    for layer in reversed(layers[:-1]):
        if layer['mode'] == 'fc':
            ...
            if flattend:
                dA_prev = (dA_prev.T).reshape(m,1,1,layer['n_C']) # flatten backward
                flattend = False
            
            dZ = backward_relu(dA_prev, layer['Z'])
...
def train(X_train, Y_train, X_test, Y_test, layers, batch_size=10, num_epoch=1, learning_rate=0.01):
    ...
    for epoch in range(num_epoch):
        ...
        pred_train, _, _= forward_propogation(X_train[:1000], layers)
        loss_train = calculate_cost(pred_train, Y_train[:1000].T)
        accuracy_train = calculate_accuracy(pred_train, Y_train[:1000].T)
