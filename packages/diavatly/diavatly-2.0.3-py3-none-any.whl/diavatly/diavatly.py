...
def blocks_from_rc(rows, columns, xo=0, yo=0):
    """
...
def model_map(polygons=None,
    ...
    if res.any():
        ...
        if not levels.any():
            ...
            boundaries = np.append(boundaries, boundaries.max()*1.1)  # Adding one bound more to have enough intervals
            cols = colors.ListedColormap([cmap(v) for v in nlv])
            cmap = cols
            norm = colors.BoundaryNorm(boundaries, cols.N)
            formatter = None

        ticks = [round(v, 1) for v in itv]
