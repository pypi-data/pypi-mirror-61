...
WAGTAIL_BLOCKS_FIELDS = {
    ...
    wagtail_block.BooleanBlock: "bool",
...
WAGTAIL_COMPOSITE_BLOCKS = ["StructBlock", "ListBlock", "StreamBlock", "StreamField"]
...
class SerializerAdapter(BaseAdapter):
    ...
    def get_all_serializer_data(cls, additional_serializers=[]):
        ...
        data = [
            cls(serializer).get_transformed_data()
...
if WAGTAIL_SUPPORT:
    class WagtailAdapter:
        ...
        def get_all_serializer_data(cls, additional_serializers=[]):
            ...
            #data = cls(rel_m[0]).get_transformed_data()
            return_data = []
        ...
        def get_transformed_data(self):
            ...
            nested.append({"name": self.wagtail_model.__name__, "fields": data, "stream": self.wagtail_model.__name__ == "StreamField"})
        ...
        def transform_stream_field(self, field, name=None):
            ...
            if type_data is None:
                for key, val in WAGTAIL_FIELDS.items():
        ...
        def stream_field_parser(self, block):
            ...
            if issubclass(block_data.__class__, wagtail_block.StreamBlock):
                ...
                child_name = f"{name.capitalize()}Child"
        ...
        def stream_field_decompose(self, stream_field):
            ...
            block_list = [block for block in stream_field.stream_block.child_blocks.items()]
            stream = []

            for block in block_list:
                stream_block = self.stream_field_parser(block)
                # logger.critical(pformat(stream_block))
                # logger.critical("----------------------")
        ...
        def transform_field(self, field):
            ...
            if data["type"] in WAGTAIL_COMPOSITE_FIELDS:
                ...
                for child_block in streams[1]:
                    other_models.append(child_block)
                    types.append(child_block["name"])
