...
if WAGTAIL_SUPPORT:
    ...
    class WagtailAdapter(SerializerAdapter):
        ...
        def get_transformed_data(self):
            ...
            for field in fields:
                if getattr(field, "serializer", False):
                    f = (field.name, field.serializer)
                else:
                    f = (field.name, getattr(self.wagtail_model, field.name).field)
                field_data = self.transform_field(f)
                data[f[0]] = field_data[0]
                nested += field_data[1]
        ...
        def transform_field(self, field_data):
            field = field_data[1]
            if field.__class__.__name__ == "ImageRenditionField":
                return [
                    {
                        "name": field_data[0],
                        "read_only": True,
