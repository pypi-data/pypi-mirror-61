...
def connect(url, motorcortex_types, param_tree, **kwargs):
    ...
    if not req.connect("%s:%s" % (address, req_port), **kwargs).get():
        raise RuntimeError("Failed to establish request connection")
    # Open subscribe connection
    sub = Subscribe(req, motorcortex_types)
    if not sub.connect("%s:%s" % (address, sub_port), **kwargs).get():
        raise RuntimeError("Failed to establish subscribe connection")
    # Login
    ...
    if login_reply_msg.status == motorcortex_msg.OK:
        raise RuntimeError("Login failed, status: %s" % login_reply_msg.status)
