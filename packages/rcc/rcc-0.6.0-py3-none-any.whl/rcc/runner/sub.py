...
async def subscriber(redisClient: RedisClient, channel: str, pattern: str, timeout: int):
    ...
    obj = []
    if pattern:
        task = asyncio.create_task(redisClient.psubscribe(pattern, cb, obj), name='foo')
    else:
        task = asyncio.create_task(redisClient.subscribe(channel, cb, obj), name='bar')
