...
async def runMigration(src_addr, dst_addr, slot):
    ...
    return await migrateSlot(masterNodes, slot, sourceNode, destinationNode)
...
@click.argument('slot')
def migrate(src_addr, dst_addr, slot):
    asyncio.run(runMigration(src_addr, dst_addr, slot))
