...
class Pitch:
    ...
    def plot_pn(self, ax):
        ...
        pass_connection = [np.sqrt(x / sum_pass) for x in num_pass]
        n = 0
        for pl1 in range(len(ids)):
            ...
            for pl2 in range(pl1 + 1, len(ids)):
                ...
                ax.plot([pl1_locs[0],pl2_locs[0]], [pl1_locs[1],pl2_locs[1]], alpha = con, c = 'black',linewidth = 4.0, zorder=1)
                ax.scatter([pl1_locs[0],pl2_locs[0]], [pl1_locs[1],pl2_locs[1]], c='red', zorder=2)
                n += 1
