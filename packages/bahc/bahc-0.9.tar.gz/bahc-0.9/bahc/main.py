...
def filterCovarianceHO(x,Norder=3,Nboot=100,var_in_sample=False):
	N,T = x.shape
	rT = range(T)
	ns = noise( N,T ) 
	...
	if var_in_sample==True:
		Cin = np.corrcoef(x)
		Ls_in = [np.dot(np.dot(v.T,Cin),v).diagonal() for v in V]
		return sum(np.dot(np.dot(v,np.diag(l)),v.T)*np.outer(s,s) for v,l,s in zip(V,Ls,std)) /Nboot, sum(np.dot(np.dot(v,np.diag(l)),v.T)*np.outer(s,s) for v,l,s in zip(V,Ls_in,std)) /Nboot
...
if __name__=='__main__':
	...
	print(filterCovarianceHO(x,var_in_sample=False))
'''
