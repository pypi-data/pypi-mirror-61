...
def get_config_register(device, next_reload=False, output=None):
    ...
    try:
        ...
        config_register = boot_out.get('active').get('configuration_register')
        if (not next_reload) or (next_reload and config_register == '0x2102'):
            return config_register
        else:
            if 'next_reload_configuration_register' in boot_out.get('active'):
                return boot_out.get('active').\
                                get('next_reload_configuration_register')
            else:
                log.error("Next-reload config-register not found")
                return None
