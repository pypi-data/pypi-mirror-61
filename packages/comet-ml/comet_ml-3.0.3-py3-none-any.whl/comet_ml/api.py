...
from .utils import clean_and_check_root_relative_path, makedirs
...
class APIExperiment(object):
    ...
    def get_model_data(self, name):
        ...
        """
        return_keys = ["fileName", "fileSize", "link", "createdAt", "metadata"]
        assets = self.get_asset_list("model-element")
        assets = [asset for asset in assets if asset["dir"] == "models/%s" % name]
        return [{key: asset[key] for key in return_keys} for asset in assets]

    def download_model(self, name, output="./"):
        """
        ...
        for data in model_data:
            ...
            # Ensure the filename won't fall outside of the given output root
            # directory
            try:
                final_path = clean_and_check_root_relative_path(root, filename)
            except ValueError:
                LOGGER.warning(
                    "    ignoring %r as it would be written outside of %r",
                    filename,
                    root,
                )
                continue

            # make any dirs on the path needed:
            makedirs(os.path.dirname(final_path), exist_ok=True)
            # and write the file:
            f = requests.get(data["link"], stream=True)
            LOGGER.info("    saving '%s'...", final_path)
        ...
        return list(
            set(
                [
                    asset["dir"].split("/", 1)[1]
                    for asset in assets
                    if asset["dir"].startswith("models/")
                ]
            )
        )
...
class API(object):
    ...
    * API.get_experiments(WORKSPACE, PROJECT_NAME, PATTERN): get all APIExperiments in WORKSPACE/PROJECT/PATTERN
    ...
    def set_project_notes(self, workspace, project_name, notes):
        ...
        project_json = self.get_project(workspace, project_name)
        project_id = project_json["projectId"]
        return self._client.set_project_notes_by_id(project_id, str(notes))
