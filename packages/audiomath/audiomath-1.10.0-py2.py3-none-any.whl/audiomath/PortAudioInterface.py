...
class Player( GenericInterface.GenericPlayer ):
	...
	def __init__( self, sound, device=None, stream=None, bufferLengthMsec=None, minLatencyMsec='auto', resample=False, verbose=None, **kwargs ):
		"""
		Args:
		...
		if SINGLE_OUTPUT_STREAM:
			if not isinstance( stream, Stream ):
				...
				else: stream = Stream( device=deviceIndex, bufferLengthMsec=bufferLengthMsec, minLatencyMsec=minLatencyMsec, verbose=verbose )
			...
			self.__stream = stream if stream else SINGLE_OUTPUT_STREAM if alreadyInitialized else Stream( device=deviceIndex, bufferLengthMsec=bufferLengthMsec, minLatencyMsec=minLatencyMsec, verbose=verbose )
			...
			self.__stream = stream if isinstance( stream, Stream ) else Stream( device=deviceIndex, bufferLengthMsec=bufferLengthMsec, minLatencyMsec=minLatencyMsec, verbose=verbose )
	...
	def __init__( self, seconds, device=None, stream=None, start=True, loop=False, verbose=None, filename=None, **kwargs ):
		...
		if SINGLE_INPUT_STREAM:
			...
			if not isinstance( stream, Stream ):
				...
				else: stream = Stream( device=stream, mode='i', verbose=verbose )
			#if alreadyInitialized and stream and stream is not SINGLE_INPUT_STREAM: raise RuntimeError( 'cannot create multiple Streams' )
			self.__stream = stream if stream else SINGLE_INPUT_STREAM if alreadyInitialized else Stream( mode='i', verbose=verbose )
			if not alreadyInitialized: SINGLE_INPUT_STREAM = self.__stream
		else:
			if stream is None: stream = DEFAULT_INPUT_DEVICE
			self.__stream = stream if isinstance( stream, Stream ) else Stream( device=stream, mode='i', verbose=verbose )
