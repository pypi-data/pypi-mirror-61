...
class _CuckooTable:
    ...
    def remove(self, item_fingerprint):
        ...
        self.bucket = np.delete(self.bucket, np.argwhere(self.bucket==item_fingerprint))
        return True
...
class CuckooFilter:
    ...
    def get_indices_and_finger(self, string_item):
        ...
        index_2 = (index_1 ^ self.obtain_index_from_hash(hash_value)) % self._filter_capacity
    ...
    def add(self, item_to_insert):
        ...
        for swap in range(self._num_swaps):
            item_fingerprint = self.table[
                random_index].swap_fingerprints(finger)
