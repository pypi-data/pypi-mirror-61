...
def dimer_method(geoms, calc_getter, N_init=None,
    ...
    def cbm_rot_force_getter(coords1, f1, f2, N):
        return get_f_perp(f1, f2, N)
    def cbm_restrict(coords1, step, coords0=coords0, dR=dR):
        # print("restricting cbm step")
    ...
    for i in range(max_cycles):
        ...
        for j in range(max_rots):
            ...
            # print(f"rms(rot_force)={get_rms(rot_force):.6f}, max(|rot_force|)={np.abs(rot_force).max():.6f}")
            ...
            # rot_step, rot_force = rot_cbm(coords1, N, f1, f2)
            # import pdb; pdb.set_trace()
            rot_force = get_f_perp(f1, f2, N)
            rfnorm = np.linalg.norm(rot_force)
            # print(f"norm(rotforce)={rfnorm:.4f}")
            # CBM break criterion
            if get_rms(rot_force) < 0.002:
                break
            rot_step = 0.1*rot_force
            coords1_rot = coords1 + rot_step
            # Constraint cooords1 back on hypersphere
            dR1_ = coords1_rot - coords0
            coords1_rot = dR1_ / np.linalg.norm(dR1_) * dR
            coords1_dir = coords1_rot - coords0
            coords1_dir /= np.linalg.norm(coords1_dir)
            coords1_rot = coords0 + coords1_dir*dR
            # coords1_rot = coords1 + rot_step
