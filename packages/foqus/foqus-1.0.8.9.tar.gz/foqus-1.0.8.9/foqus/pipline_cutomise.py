...
from Augmentor.ImageUtilities import scan_directory, scan, scan_dataframe, AugmentorImage
...
class Pipeline2(object):
    ...
    def rotate90(self, probability):
        ...
        if not 0 < probability <= 1:
            raise ValueError(Pipeline2._probability_error_text)
        else:
    ...
    def resize(self, probability, width, height, resample_filter="BICUBIC"):
        ...
        elif resample_filter not in Pipeline2._legal_filters:
            raise ValueError("The save_filter argument must be one of %s." % Pipeline2._legal_filters)
        else:
...
class DataFramePipeline(Pipeline2):
