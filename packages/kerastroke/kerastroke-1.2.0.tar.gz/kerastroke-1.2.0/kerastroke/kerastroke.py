...
class Stroke(Callback):
    ...
    def __init__(self, minweight=-.05, maxweight=.05, volatility_ratio=.05, decay=None, index=None, verbose=False, pruning=True, pruningmin=0.0, pruningmax=0.2):
    ...
    def on_epoch_end(self, batch, logs=None):
        if(self.index is None):
            ...
            num_weights = len(nditer(weights))
        ...
        if(self.pruning):
            weights = self.model.get_layer(index=self.index).get_weights()[0].copy()
            biases = self.model.get_layer(index=self.index).get_weights()[1]

            new_weights = np.empty_like(weights)
            for weight in nditer(new_weights, op_flags = ['readwrite']):
