...
class GetMetadata:
    ...
    def __init__(self, _env_name=None):
        """
        Parameters
        ----------
        headers : dict
            Contain Olive API key for accessing the Static Vessel Data API, requires .olive_env containing API Key
        ...
        if _env_name:
            dotenv_path = find_dotenv(filename=_env_name, usecwd=True)

        else:
            dotenv_path = find_dotenv(filename=".olive_env", usecwd=True)
        ...
        try:
            ...
            msg = "Could not find Olive API Key in .olive_env file, please refer to section 'configuring the Olive " \
                  "API Key' in README"
            raise KeyError(msg)
    ...
    def get_current_attributes(self, mmsi):
        ...
        try:
            ...
            if status == 400:
                ...
                raise HTTPError("Please ensure that you are configured with the Olive API Key, refer to README")
    ...
    def get_timestamps(self, mmsi):
        ...
        try:
            # get results from api endpoint using specified parameters
            url = '{endpoint}/{mmsi}'.format(endpoint=self.timestamps_endpoint, mmsi=mmsi)
