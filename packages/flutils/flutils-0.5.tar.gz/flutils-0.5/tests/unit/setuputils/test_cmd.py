...
class TestCmd(unittest.TestCase):
    ...
    def test_render() -> None:
        with patch('sys.stdin', autospec=True) as stream:
            stream.readline.side_effect = [
                'A line of text',
                '',
            ]
            with patch('sys.stdout', autospec=True) as output:
                output.write.return_value = None
                _reader(stream, output)
                stream.readline.assert_has_calls([call(), call()])
    ...
    def test_execute(self) -> None:
        ...
        type(popen_obj).stdout = PropertyMock(return_value=None)
        type(popen_obj).stderr = PropertyMock(return_value=None)

        patcher = patch(
            'flutils.setuputils.cmd.Popen',
            return_value=popen_obj
        )
        popen = patcher.start()
        ...
        with patch('flutils.setuputils.cmd.Thread') as thread:
            thread.start.side_effect = [None, None]
            ret = _execute(command)

            self.assertEqual(ret, 0)

            thread.return_value.start.assert_has_calls([call(), call()])
    ...
    def test_show_command() -> None:
        ...
        with patch('sys.stdout', autospec=True) as stdout:
            stdout.return_value.write.side_effect = [
                None,
                None,
                None,
                None,
    ...
    def test_run__0(self) -> None:
        ...
        patcher = patch('flutils.setuputils.cmd._execute', side_effect=[0, 0])
        execute = patcher.start()
        self.addCleanup(patcher.stop)
        ...
        show_command.assert_has_calls([
            call(cmds[0], sys.stdout),
            call(cmds[1], sys.stdout)
        ])
        execute.assert_has_calls([
            call(cmds[0], out=sys.stdout, err=sys.stderr),
            call(cmds[1], out=sys.stdout, err=sys.stderr)
        ])
    ...
    def test_run__1(self) -> None:
        ...
        patcher = patch('flutils.setuputils.cmd._execute', side_effect=[0, 5])
        execute = patcher.start()
        self.addCleanup(patcher.stop)
