...
def test_read_mol():
    ...
    assert len(ref_coordinates) == len(coordinates) == len(charges) == len(masses)

    for i in range(len(coordinates)):
        assert len(ref_coordinates[0]) == len(coordinates[i])

        for j in range(len(coordinates[0])):
            assert ref_coordinates[i][j] == coordinates[i][j]

    for i in range(len(charges)):
        assert ref_charges[i] == charges[i]
        assert ref_masses[i] == masses[i]
...
def test_project_out_transl_and_rot():
    ...
    assert len(ref_vib_hess) == len(vib_hess)

    for i in range(len(vib_hess)):
        assert len(ref_vib_hess[0]) == len(vib_hess[i])

        for j in range(len(vib_hess[i])):
            assert ref_vib_hess[i][j] == pytest.approx(vib_hess[i][j], 1.0e-6)
...
def test_get_vib_harm_freqs_and_eigvecs():
    ...
    vib_energies, transformation_matrix, num_coordinates = \
                                       get_vib_harm_freqs_and_eigvecs(mol_name, hess, True, 0)

    assert ref_num_coordinates == num_coordinates

    assert len(ref_vib_energies) == len(vib_energies)

    for i in range(len(vib_energies)):
        assert ref_vib_energies[i] == vib_energies[i]

    assert len(ref_T) == len(transformation_matrix)

    for i in range(len(transformation_matrix)):
        assert len(ref_T[0]) == len(transformation_matrix[i])

        for j in range(len(transformation_matrix[0])):
            assert ref_T[i][j] == pytest.approx(transformation_matrix[i][j], 1.0e-8)
    ...
    vib_energies, transformation_matrix, num_coordinates = \
                                       get_vib_harm_freqs_and_eigvecs(mol_name, hess, False, 0)
...
def test_get_vibrational_w():
    ...
    assert len(ref_vib_w) == len(vib_w)
    for i in range(len(vib_w)):
        assert ref_vib_w[i] == pytest.approx(vib_w[i], 1.0e-40)
    ...
    assert len(ref_vib_w) == len(vib_w)
    for i in range(len(vib_w)):
        assert ref_vib_w[i] == pytest.approx(vib_w[i], 1.0e-9)
