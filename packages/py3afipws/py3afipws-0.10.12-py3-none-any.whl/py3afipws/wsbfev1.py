...
class WSBFEv1(BaseWS):
    """Interfaz para el WebService de Bono Fiscal Electrónico V1 (FE Bs.
    Capital)"""
    _public_methods_ = [
    ...
    def __analizar_errores(self, ret):
        ...
        if 'BFEErr' in ret:
            ...
            self.ErrCode = ' '.join([str(error['ErrCode'])
                                     for error in errores])
            self.ErrMsg = '\n'.join(self.Errores)
        if 'BFEEvents' in ret:
            events = [ret['BFEEvents']]
            self.Eventos = ['%s: %s' % (evt['EventCode'], evt.get(
                'EventMsg', "")) for evt in events]
    ...
    def CrearFactura(self, tipo_doc=80, nro_doc=23111111113,
                     ...
                     imp_moneda_ctz=1.0, fecha_venc_pago=None, **kwargs):
        ...
        fact = {
            ...
            'zona': zona, 'tipo_doc': tipo_doc,
            'nro_doc':  nro_doc, 'imp_total': imp_total,
            'imp_neto': imp_neto, 'impto_liq': impto_liq,
            'impto_liq_rni': impto_liq_rni, 'imp_op_ex': imp_op_ex,
            'imp_tot_conc': imp_tot_conc, 'imp_perc': imp_perc,
            'imp_perc_mun': imp_perc_mun, 'imp_iibb': imp_iibb,
            'imp_internos': imp_internos, 'imp_moneda_id': imp_moneda_id,
            'imp_moneda_ctz': imp_moneda_ctz,
            'fecha_venc_pago': fecha_venc_pago,
            'cbtes_asoc': [], 'opcionales': [],
            'iva': [], 'detalles': [],
        }
        self.factura = fact
        return True
    ...
    def GetCMP(self, tipo_cbte, punto_vta, cbte_nro):
        ...
        if 'BFEResultGet' in result:
            ...
            # Obs, cae y fecha cae
            if 'Cae' in resultget:
                self.Obs = resultget['Obs'] and resultget['Obs'].strip(
                    " ") or ''
                self.CAE = resultget['Cae']
                vto = str(resultget['Fch_venc_Cae'])
                self.Vencimiento = "%s/%s/%s" % (vto[6:8], vto[4:6], vto[0:4])
                # .strftime("%Y/%m/%d")
                self.FechaCbte = resultget['Fecha_cbte_orig']
                # .strftime("%Y/%m/%d")
                self.FechaCAE = resultget['Fecha_cbte_cae']
    ...
    def GetLastCMP(self, tipo_cbte, punto_vta):
        ...
        if 'BFEResult_LastCMP' in result:
            ...
            self.FechaCbte = resultget.get(
                'Cbte_fecha')  # .strftime("%Y/%m/%d")
            return self.CbteNro
    ...
    def GetParamUMed(self):
        ...
        for u in result['BFEResultGet']:
            ...
            umeds.append(umed)
        return [
            '%(id)s: %(ds)s (%(vig_desde)s - %(vig_hasta)s)' % p for p in umeds
        ]
    ...
    def GetParamMon(self):
        ...
        for m in result['BFEResultGet']:
            ...
            mons.append(mon)
        return [
            '%(id)s: %(ds)s (%(vig_desde)s - %(vig_hasta)s)' % p for p in mons]
    ...
    def GetParamTipoCbte(self):
        ...
        for t in result['BFEResultGet']:
            ...
            try:
                ...
                pass
        return [
            '%(id)s: %(ds)s (%(vig_desde)s - %(vig_hasta)s)' % p for p in tipos
        ]
    ...
    def GetParamNCM(self):
        ...
        for n in result['BFEResultGet']:
            ...
            try:
                ...
                pass
        return [
            u'%(id)s: %(ds)s (%(vig_desde)s - %(vig_hasta)s)' % p for p in ncms
        ]
    ...
    def GetParamZonas(self):
        ...
        for z in result['BFEResultGet']:
            ...
            try:
                ...
                pass
        return [
            '%(id)s: %(ds)s (%(vig_desde)s - %(vig_hasta)s)' % p for p in zonas
        ]
...
if __name__ == "__main__":
    ...
    if "--register" in sys.argv or "--unregister" in sys.argv:
        ...
        # Crear objeto interface Web Service de Factura Electrónica de
        # Exportación
        wsbfev1 = WSBFEv1()
        ...
        if "--prueba" in sys.argv:
            try:
                ...
                tipo_cbte = '--nc' in sys.argv and 3 or 201
                ...
                ok = wsbfev1.CrearFactura(
                    tipo_doc, nro_doc, zona, tipo_cbte, punto_vta, cbte_nro,
                    fecha_cbte, imp_total, imp_neto, impto_liq, imp_tot_conc,
                    impto_liq_rni, imp_op_ex, imp_perc, imp_iibb,
                    imp_perc_mun, imp_internos, imp_moneda_id, imp_moneda_ctz,
                    fecha_venc_pago)
