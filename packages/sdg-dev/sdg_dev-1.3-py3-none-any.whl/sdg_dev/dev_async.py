...
class DevAsync(DevMaster):
    ...
    def sendto(self, addr, msg, ackfrmt='raw', timeout=None, remix=None):
        ...
        if type(c.ack) is DevBadCode:
            raise DevException(f"DevException '{self._name}' Received", badcode=c.ack)
        if c.ack is None:
            raise DevException(f"DevException '{self._name}' No answer for cmd{c.name}!")
    ...
    def _incmd_parser(self, rx):
        ...
        return self._addr + bytes([0xF0, rx[0], abs(DevBadCode.UNCNOWN)])
