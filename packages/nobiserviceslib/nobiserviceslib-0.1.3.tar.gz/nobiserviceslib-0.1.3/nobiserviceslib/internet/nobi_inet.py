...
class ConnectionManager:
    ...
    def spectrum_predict(self, model_path, data, predict_mode, buffer_size=4096, out_dir='.', parm: dict = None):
        ...
        if predict_mode == 0:
            ...
            if parm is not None:
                try:
                    prediction_job = str(parm['id'])
                except Exception:
                    raise SocketError(SocketErrorCases.SPECTRUM_PREDICT_PARAMETER_ERROR)
            else:
                raise SocketError(SocketErrorCases.SPECTRUM_PREDICT_PARAMETER_ERROR)
            servo_dir = model_path
            prediction_file = data
            username = self.__c_s.get_username()
            # servo_dir = "nfs-1:/prediction_jobs/id-11/train-1581776914545/Servo".encode('utf-8')
            # prediction_job = "nfs-1:/predict-1581757769200".encode('utf-8')
            # prediction_file = "nfs-1:/prediction_jobs/id-11/train-1581776914545/predict-1581776914545/test.csv".encode('utf-8')
            # username = "Guava&Nobi".encode('utf-8')
