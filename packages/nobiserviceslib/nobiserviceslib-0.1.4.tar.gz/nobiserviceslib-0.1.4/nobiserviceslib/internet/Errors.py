...
class SocketErrorCases(Enum):
    ...
    SPECTRUM_PREDICT_DATA_TYPE_ERROR = 12
    SET_TENSORFLOW_MODEL_ERROR = 13
    SPECTRUM_PREDICT_FAILED = 14
    SPECTRUM_PREDICT_PARAMETER_ERROR = 15
...
class SocketError(Exception):
    def __init__(self, case, note=""):
        ...
        elif case == SocketErrorCases.VIEW_PPG_ERROR:
        ...
        elif case == SocketErrorCases.SPECTRUM_PREDICT_DATA_TYPE_ERROR:
            self.__message = "Data type not match."
        elif case == SocketErrorCases.SET_TENSORFLOW_MODEL_ERROR:
            self.__message = "Set tensorflow model has failed."
        elif case == SocketErrorCases.SPECTRUM_PREDICT_FAILED:
            self.__message = "Spectrum predict has failed."
        elif case == SocketErrorCases.SPECTRUM_PREDICT_PARAMETER_ERROR:
            self.__message = "Unknown parameter at Spectrum prediction."
        else:
