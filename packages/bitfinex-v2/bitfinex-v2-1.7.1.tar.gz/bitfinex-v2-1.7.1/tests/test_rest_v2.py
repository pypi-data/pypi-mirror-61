...
def test_1_liquidation_feed_url_is_ok(client, requests_mock):
    ...
    client.liquidation_feed()
    assert requests_mock.request_history[0].url == (
        'https://api-pub.bitfinex.com/v2/liquidations/hist'
    )

def test_2_liquidation_feed_url_is_ok(client, requests_mock):
    response_text = json.dumps([None])
    requests_mock.register_uri(
        rmock.ANY,
        rmock.ANY,
...
def test_order_update_url_is_ok(client, requests_mock):
    response_text = json.dumps([])
    requests_mock.register_uri(
        rmock.ANY,
        rmock.ANY,
    ...
    client.order_update(38646826900, price="2.02", amount="-98", flags=64)
    assert requests_mock.request_history[0].url == (
        'https://api.bitfinex.com/v2/auth/w/order/update'
    )

def test_cancel_order_url_is_ok(client, requests_mock):
    response_text = json.dumps([])
    requests_mock.register_uri(
        rmock.ANY,
        rmock.ANY,
    ...
    client.cancel_order(id=38646826900)
    assert requests_mock.request_history[0].url == (
