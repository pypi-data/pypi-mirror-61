    ...
    def on_epoch_end(self, training_context):
        try:
            if (self.generator_first and training_context['gan_role'] == 'discriminator') or (  not self.generator_first and training_context['gan_role'] == 'generator'):
                ...
                grad1 = np.abs(np.array( self.training_items.value_list[0].training_context['grads_state']['last_layer'][-3:])).mean()
                grad2 = np.abs(np.array( self.training_items.value_list[1].training_context['grads_state']['last_layer'][-3:])).mean()
