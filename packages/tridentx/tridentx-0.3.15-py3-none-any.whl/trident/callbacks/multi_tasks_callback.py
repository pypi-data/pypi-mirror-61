    ...
    def on_batch_end(self, training_context):
        if training_context['gan_role'] == 'generator':
            if (training_context['current_epoch'] * training_context['total_batch'] + training_context[ 'current_batch'] + 1) % self.tile_image_frequency == 0:
                ...
                self.tile_images = []
            if (training_context['current_epoch'] * training_context['total_batch'] + training_context[ 'current_batch'] + 1) >100:
                metric1=np.array(training_context['metrics'].value_list[0][-3:]).mean()
                if self.generator_best_metric is None:
                    self.generator_best_metric=metric1
                    self.generator_worse_metric=metric1
                elif metric1 > self.generator_best_metric:
                    self.generator_best_metric = metric1
                    self.generator_best_epoch = training_context['current_epoch'] * training_context['total_batch'] + training_context[ 'current_batch']+1
                elif metric1 < self.generator_worse_metric:
                    self.generator_worse_metric = metric1
                    print('generator_worse_metric',metric1)


        elif training_context['gan_role'] == 'discriminator':
            if (training_context['current_epoch'] * training_context['total_batch'] + training_context[ 'current_batch'] + 1) > 100:
                metric2 = np.array(training_context['metrics'].value_list[0][-3:]).mean()
                if self.discriminator_best_metric is None:
                    self.discriminator_best_metric = metric2
                    self.discriminator_worse_metric = metric2
                elif metric2 < self.discriminator_best_metric:
                    self.discriminator_best_metric = metric2
                    self.discriminator_best_epoch = training_context['current_epoch'] * training_context['total_batch'] + training_context[ 'current_batch']+1
                if metric2 > self.discriminator_worse_metric:
                    self.discriminator_worse_metric = metric2
                    print('discriminator_worse_metric',metric2)
