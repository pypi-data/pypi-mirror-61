        ...
        if training_context['gan_role'] == 'generator':
            ...
            try:
                ...
                if self.gan_type == 'gan':
                    ...
                    D_fake_logit = sigmoid(self.D_fake - self.D_real.mean(0, True))
                    this_loss = (CrossEntropyLoss()(D_real_logit, true_label).mean() + CrossEntropyLoss()(D_fake_logit,false_label).mean()) / 2
    ...
    def on_epoch_end(self, training_context):
        try:
            if (self.generator_first and training_context['gan_role'] == 'discriminator') or (
                ...
                # if generator_grads < 2e-3 and discriminator_grads < 2e-4 and self.discriminator_best_epoch < training_context[ 'current_epoch'] - 3 and self.generator_best_epoch < training_context['current_epoch'] - 3:  #  #     if abs(self.generator_best_metric - generator_metric) > 0.2 and abs(  #             self.discriminator_best_metric - discriminator_metric) > 0.2:  #         self.training_items.value_list[0].optimizer.adjust_learning_rate(  #             self.training_items.value_list[0].optimizer.lr / 2, True)  #         self.training_items.value_list[1].optimizer.adjust_learning_rate(  #             self.training_items.value_list[1].optimizer.lr / 2, True)  #         self.noise_end_epoch = training_context['current_epoch'] + 10  #         self.experience_replay = False  #         self.noisy_labels = True
