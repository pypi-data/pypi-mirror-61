        ...
        if self.model.built:
            ...
            if len(boxes_list) > 0:
                ...
                if boxes is not None:
                    ...
                    score = to_numpy(boxes[:, 4]).reshape(-1)
                    boxes = boxes[score>  self.detection_threshould[1]]
                    ...
                    score=boxes[:,4]
                    cutoff=get_cutoff(score)
                    boxes = boxes[score > self.detection_threshould[2]]

                    #######################################
                    #########onet finish
                    #######################################
                    boxes=nms(boxes, threshold=0.4,image_size=(img.shape[0],img.shape[1]),min_size=self.minsize)
