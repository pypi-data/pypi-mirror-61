...
class GanCallback(GanCallbacksBase):
        ...
        if training_context['gan_role'] == 'generator':
            ...
            try:
                ...
                if self.gan_type == 'gan':
                    ...
                    D_fake_logit = torch.sigmoid(self.D_fake - torch.mean(self.discriminator(self.img_real),0,False))
    ...
    def on_batch_end(self, training_context):
        if training_context['gan_role'] == 'generator':
            if (training_context['current_epoch'] * training_context['total_batch'] + training_context[ 'current_batch'] + 1) % self.tile_image_frequency == 0:
                ...
                self.tile_images = []
        if training_context['gan_role'] == 'generator' and self.noised_lr and training_context['current_epoch'] * training_context['total_batch'] + training_context['current_batch'] + 1 > 1000:
            factor = math.cos(math.pi * (training_context['current_epoch'] * training_context['total_batch'] + training_context['current_batch'] + 1) / 100.0)
            base_lr = training_context['base_lr']
            training_context['optimizer'].adjust_learning_rate(base_lr * (1 + 0.2 * factor + 0.3 * (random.random() - 0.5)), False)
        if  training_context['gan_role'] == 'discriminator' and  training_context['current_epoch'] * training_context['total_batch'] + training_context['current_batch'] + 1==12000:
            training_context['base_lr']=training_context['base_lr']/2
        if training_context['gan_role'] == 'discriminator' and self.noised_lr and training_context['current_epoch'] * training_context['total_batch'] + training_context['current_batch'] + 1 > 1000:
            factor = math.cos(math.pi * (training_context['current_epoch'] * training_context['total_batch'] + training_context['current_batch'] + 1) / 100.0)
            base_lr = training_context['base_lr']
            training_context['optimizer'].adjust_learning_rate(base_lr * (1 + 0.1 * factor + 0.1 * (random.random() - 0.5)), False)
    ...
    def on_epoch_end(self, training_context):
        try:
            if (self.generator_first and training_context['gan_role'] == 'discriminator') or (  not self.generator_first and training_context['gan_role'] == 'generator'):
                pass

                # if generator_grads < 2e-3 and discriminator_grads < 2e-4 and self.discriminator_best_epoch < training_context[ 'current_epoch'] - 3 and self.generator_best_epoch < training_context['current_epoch'] - 3:
                #
...
class CycleGanCallback(GanCallbacksBase):
    ...
    def on_training_start(self, training_context):
        ...
        conterparty['generatorA'].optimizer.param_groups[0]['params']=itertools.chain(self.generatorA.trainable_weights,self.generatorB.trainable_weights)
        #conterparty['generatorB'].optimizer.param_groups[0]['params'] = itertools.chain(self.generatorA.trainable_weights, self.generatorB.trainable_weights)
    ...
    def on_data_received(self, training_context):
        ...
        try:
            ...
            if training_context['gan_role'] == 'generatorA' or training_context['gan_role'] == 'generatorB':
                ...
                realA=self.realA
                if self.noised_real:
                    realA = (self.realA  + to_tensor(  self.noise_intensity* (1 - float(curr_epochs) / (tot_epochs)) * np.random.standard_normal( list(self.realA.size())))).clamp(-1, 1)
                ...
                realB = self.realB
                if self.noised_real:
                    realB = (self.realB + to_tensor(self.noise_intensity * (1 - float(curr_epochs) / (tot_epochs)) * np.random.standard_normal( list(self.realB.size())))).clamp(-1, 1)
    ...
    def post_loss_calculation(self, training_context):
        ...
        if training_context['gan_role'] == ['generatorA' , 'generatorB']:
            try:
                ...
                training_context['current_loss'] = training_context['current_loss'] +self.identity_loss_weight*loss_identity
                ...
                training_context['current_loss'] = training_context['current_loss'] +self.cycle_loss_weight* loss_cycle
            ...
            try:
                ...
                if self.gan_type == 'vanilla':
                    ...
                    def compute_gradient_penalty():
                        ...
                        # Get gradient w.r.t. interpolates
                        gradients = \
                        torch.autograd.grad(outputs=out, inputs=interpolates, grad_outputs=fake, create_graph=True,   retain_graph=True, only_inputs=True, )[0]
                        gradients = gradients.view(gradients.size(0), -1)
                        gradient_penalty = ((gradients.norm(2, dim=1) - 1) ** 2).mean()
                        return gradient_penalty
        ...
        if (self.generator_first and training_context['gan_role'] == 'discriminator') or ( not self.generator_first and training_context['gan_role'] == 'generator'):
            if (training_context['current_epoch'] + 1) % 10 == 0:
                if training_context['optimizer'].lr > 1e-6:
                    if np.array(training_context['grads_state']['last_layer'][-10:]).mean()<2e-3 and np.array(training_context['grads_state']['last_layer'][-10:]).mean()<2e-3:
                        training_context['optimizer'].adjust_learning_rate(training_context['optimizer'].lr * 0.5, True)  # elif training_context['current_epoch']>=1 and float(self.D_real.mean()) > 0.8 and float(self.D_fake.mean()) < 0.1 :  #     if self.discriminator is not None and model.name == self.discriminator.name:  #         training_context['optimizer'].adjust_learning_rate(training_context['optimizer'].lr / 2.0)
