...
def inverted_residual(in_filters,num_filters=64,strides=1,expansion = 4,name=''):
    ...
    layers=[]
    if expansion!=1 :
        layers.append(Conv2d_Block((1,1),num_filters=mid_filters,strides=1,auto_pad=True,padding_mode=PaddingMode.zero,normalization='batch',activation='relu6',name=name + '_{0}_conv'.format(len(layers))))

    layers.append(DepthwiseConv2d_Block((3, 3), depth_multiplier=1, strides=strides, auto_pad=True,padding_mode=PaddingMode.zero, normalization='batch', activation='relu6', name=name + '_{0}_conv'.format(len(layers))))
    layers.append(Conv2d_Block((1, 1), num_filters=num_filters, strides=1, auto_pad=False, padding_mode=PaddingMode.zero, normalization='batch', activation=None, name=name + '_{0}_conv'.format(len(layers))))
...
def MobileNet( input_shape=(3, 224, 224), classes=1000, use_bias=False, width_mult=1.0,round_nearest=8, include_top=True, model_name='',
    ...
    features = []
    features.append(Conv2d_Block((3,3),num_filters=input_filters,strides=2,auto_pad=True,padding_mode=PaddingMode.zero,normalization='batch',activation='relu6',name='first_layer'))
    ...
    features.append(Conv2d_Block((1,1), last_filters,auto_pad=True,padding_mode=PaddingMode.zero,normalization='batch',activation='relu6',name='last_layer'))
...
def MobileNetV2(include_top=True,
    ...
    if pretrained==True:
        download_file_from_google_drive('1ULenXTjOO5PdT3fHv6N8bPXEfoJAn5yL',dirname,'mobilenet_v2.pth')
