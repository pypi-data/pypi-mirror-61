...
def resnet_block(num_filters=64,strides=1,activation='leaky_relu', normalization='batch',use_spectral=False,dilation=1,name=''):
    ...
    return [ShortCut2d(Sequential(Conv2d_Block((3,3),filter_rate=0.5,strides=1,auto_pad=True,padding_mode=PaddingMode.replicate,use_spectral=use_spectral,normalization=normalization,activation=activation,use_bias=False,dilation=dilation,name=name + '_0_conv'),
                                 Conv2d_Block((1, 1), filter_rate=2 , strides=1, auto_pad=True,padding_mode=PaddingMode.replicate,use_spectral=use_spectral,normalization=normalization, activation=activation,use_bias=False,name=name + '_1_conv')),
                      Identity(),activation=activation,name=name),
            Conv2d_Block((kernal, kernal), num_filters=num_filters, strides=strides, auto_pad=True, padding_mode=PaddingMode.replicate, use_spectral=use_spectral, normalization=normalization, activation=activation, use_bias=False, name=name + '_conv')]
...
def separable_resnet_block(num_filters=64,strides=1,activation='leaky_relu', normalization='batch',use_spectral=False,dilation=1,name=''):
    kernal=1 if strides==1 else 3
    return [ShortCut2d(Sequential(SeparableConv2d_Block((3,3),depth_multiplier=1,strides=1,auto_pad=True,padding_mode=PaddingMode.replicate,use_spectral=use_spectral,normalization=normalization,activation=activation,use_bias=False,dilation=dilation,name=name + '_0_conv'),
                                 Conv2d_Block((1, 1), filter_rate=2 , strides=1, auto_pad=True,padding_mode=PaddingMode.replicate,use_spectral=use_spectral,normalization=normalization, activation=activation,use_bias=False,name=name + '_1_conv')),
                      Identity(),activation=activation,name=name),
            Conv2d_Block((kernal, kernal), num_filters=num_filters, strides=strides, auto_pad=True, padding_mode=PaddingMode.replicate, use_spectral=use_spectral, normalization=normalization, activation=activation, use_bias=False, name=name + '_conv')]
...
def bottleneck_block(num_filters=64,strides=1,reduce=4,activation='leaky_relu', normalization='batch',use_spectral=False,dilation=1,name=''):
    ...
    return ShortCut2d(Sequential(Conv2d_Block((1,1),filter_rate=1,strides=1,auto_pad=True,padding_mode=PaddingMode.replicate,use_spectral=use_spectral,normalization=normalization,activation=activation,use_bias=False,name=name + '_0_conv'),
                                 Conv2d_Block((3, 3), filter_rate=1/reduce , strides=strides, auto_pad=True,padding_mode=PaddingMode.replicate,use_spectral=use_spectral,normalization=normalization, activation=activation,use_bias=False,dilation=dilation,name=name + '_1_conv'),
                                 Conv2d_Block((1,1),num_filters=num_filters,strides=1,auto_pad=True,padding_mode=PaddingMode.replicate,use_spectral=use_spectral,normalization=normalization,activation=None,use_bias=False,name=name + '_2_conv')),
...
def gan_builder(
    ...
    def build_generator():
        ...
        if upsample_mode == UpsampleMode.pixel_shuffle.value:
            layers.append(Conv2d_Block((3, 3), 256, strides=1, auto_pad=True, use_spectral=use_spectral,use_bias=False, activation=activation,normalization=generator_norm,padding_mode=PaddingMode.replicate))
        else:
            layers.append(Conv2d_Block((3, 3), 128, strides=1, auto_pad=True,use_spectral=use_spectral, use_bias=False, activation=activation,normalization=generator_norm,padding_mode=PaddingMode.replicate))
        ...
        while current_width<image_width:
            ...
            if upsample_mode==UpsampleMode.transpose.value:
                layers.append(TransConv2d_Block((3,3),filter_rate=1,strides=scale,auto_pad=True,padding_mode=PaddingMode.replicate,use_spectral=use_spectral,use_bias=False,activation=activation,normalization=generator_norm,dilation=dilation,name='transconv_block{0}'.format(i)))
            else:
            ...
            if generator_build_block == BuildBlockMode.base.value :
                layers.append(Conv2d_Block((3, 3), filter, strides=1, auto_pad=True,padding_mode=PaddingMode.replicate, use_spectral=use_spectral,use_bias=False, activation=activation, normalization=generator_norm,dilation=dilation,name='base_block{0}'.format(i)))
