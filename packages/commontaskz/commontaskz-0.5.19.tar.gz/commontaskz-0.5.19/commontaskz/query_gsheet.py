...
class QueryGSheet(Query):
    ...
    def __init__(self, key, url=None, a_range=None, sheet_id=None, sheet_name=None,
                 max_retries=3, retry_delay=timedelta(minutes=1), **kwargs):
    ...
    def query(self, full_url, **kwargs):
        logging.debug("Querying %s", full_url)
        full_url += "?key={key}".format(key=self.key)
        return requests.get(full_url, params={}, headers={})
    ...
    def run(self, url=None, a_range=None, sheet_id=None, sheet_name=None, collapse=True, **kwargs):
...
class MapIDs(QueryGSheet):
    def __init__(self, key, url=None, a_range=None, sheet_id=None, sheet_name=None, key_idx=None, value_idx=None,
                 max_retries=3, retry_delay=timedelta(minutes=1), **kwargs):
        super().__init__(key, url, a_range, sheet_id, sheet_name, max_retries, retry_delay, **kwargs)
        self.name = "Map IDS"
    ...
    def run(self, ids, url=None, a_range=None, sheet_id=None, sheet_name=None, key_idx=None, value_idx=None, **kwargs):
        full_url = self.prep(url, a_range, sheet_id, sheet_name)
        response = self.query(full_url)
        result = self.process("", full_url, response)
