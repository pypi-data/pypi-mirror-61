...
class automatorHelper(object):
    pool = urllib3.PoolManager()
    def __init__(self,ip="127.0.0.1",port=9600,timeout=10):
        self.url ="http://%s:%d/helper" % (ip,  port)
        self.timeout =timeout
        
    def __call__(self,itype,imothed=None,icontent=None):
        data = {"type": itype, "mothed": imothed,"content":icontent}
...
class shellHelper(object):
    pool = urllib3.PoolManager()
    def __init__(self,ip="127.0.0.1",port=9600,timeout=10):
        self.url ="http://%s:%d/shell" % (ip,  port)
        self.timeout =timeout
...
class NotFoundHandler(object):
    ...
    def __get__(self, instance, type):
        return self.__handlers["xx_device"]
...
class AutomatorServer(object):
    ...
    def __init__(self,server_host=None,server_port=None,ihelper=None):
        self.uiautomator_process = None
        self.device_ip   = server_host
        self.device_port = server_port
        self.helper_tool = ihelper
        self.retry=3
    # def push(self):
    ...
    @property
    def jsonrpc(self):
        return self.jsonrpc_wrap(timeout=10)
        # return self.jsonrpc_wrap(timeout=int(os.environ.get("jsonrpc_timeout", 10)))
        ...
        def _JsonRPCMethod(url, method, timeout, restart=True):
            ...
            def wrapper(*args, **kwargs):
                for i in range(server.retry):
                    URLError = urllib3.exceptions.HTTPError
                    try:
                        return _method_obj(*args, **kwargs)
                    except (JsonRPCError, URLError, socket.error, HTTPException) as e:
                        if(i<2):
                            server.stop()
                            server.start()
                        else:
...
#                         else:
#                            server.retry-=1
#                         return _JsonRPCMethod(url, method, timeout, server.retry>0)(*args, **kwargs)
#                     else:
#                         raise
...
#                              method_class=_JsonRPCMethod)
    def __jsonrpc(self):
        ...
        return JsonRPCClient(self.rpc_uri, timeout=10)
    ...
    def start(self, timeout=5):
        print("启动")
        self.helper_tool(itype="helper",imothed="restart.uiautomator")
    ...
    @property
    def stop_uri(self):
        return "http://%s:%d/s_t_o_p" % (self.device_ip, self.device_port)

    @property
    def rpc_uri(self):
        return "http://%s:%d/jsonrpc/0" % (self.device_ip, self.device_port)

    @property
    def screenshot_uri(self):
        return "http://%s:%d/screenshot/0" % (self.device_ip, self.device_port)
...
class AutomatorDevice(object):
    ...
    def __init__(self,host_ip="127.0.0.1",rpc_port=9008,helper_port=9600):
        self.helper  = automatorHelper(ip=host_ip,port=helper_port)
        self.server  = AutomatorServer(server_host=host_ip, server_port=rpc_port, ihelper=self.helper)
        self.shell   = shellHelper(ip=host_ip,port=helper_port)
        
        self.helper(itype="ime",imothed="begin")
        self.helper(itype="helper",imothed="restart.uiautomator",icontent="init")
    ...
    def app(self):
        ...
        #         result =shell("monkey -p %s -c android.intent.category.LAUNCHER  1" %package)
        #         return result['status']

        #     def stop(self,package):
        #         result =shell("am force-stop %s" %package)
        #         return result['status']

        #     def clear(self,package):
        #         result =shell("pm clear %s" %package)
        #         return result['status']

        #     def path(self,package):
        #         result =shell("pm path %s" %package)
        #         return result['msg']  
        
        helper =self.helper
        class _app(object):
            def launch(self,lable):
                helper(itype="helper",imothed="launch.app",icontent=lable)

            def stop(self,lable):
                helper(itype="helper",imothed="stop.app",icontent=lable)

            def listActive(self):
                helper(itype="helper",imothed="list.active")
...
class AutomatorDeviceUiObject(object):
    ...
    def click(self):
        ...
        # Usage:
        # d(text="inputText").input().clear() 
    @property
    def input(self):
        _slef=self
        class _ime(object):
            def clear(self):
                _slef.helper(itype="ime",imothed="clear")

            def text(self,text):
                _slef.helper(itype="ime",imothed="text",icontent=text)

            def commit(self,test):
                _slef.helper(itype="ime",imothed="text",icontent=text)
                _slef.helper(itype="ime",imothed="enter")
