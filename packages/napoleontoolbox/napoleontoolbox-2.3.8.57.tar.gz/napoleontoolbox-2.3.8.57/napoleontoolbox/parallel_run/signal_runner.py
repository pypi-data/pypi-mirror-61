...
def reconstitute_signal_perf(data=None, initial_price = 1. , average_execution_cost = 0.001):
...
class SimpleSignalRunner(AbstractRunner):
    def runTrial(self, saver, seed,  look_back_window, threshold, contravariant):
        ...
        hourly_df = roll_wrapper(hourly_df, look_back_window, skipping_size,
                                  lambda x: dd_threshold(data=x, threshold=threshold, contravariant=contravariant))
        ...
        saver.saveResults(savingKey, hourly_df[['perf_return']])
