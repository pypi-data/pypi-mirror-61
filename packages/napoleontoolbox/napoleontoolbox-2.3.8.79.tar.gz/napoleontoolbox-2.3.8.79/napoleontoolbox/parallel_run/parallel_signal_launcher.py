...
class SignalParalleLauncher():
    def __init__(self,drop_token='', dropbox_backup=True, local_root_directory='../data',user='napoleon',  db_path_suffix = '_run.sqlite', signal_types=['dd_threshold'] ,lookback_windows=[5], thresholds=[1.], contravariants=[1.], signal_idiosyncratic_paramaters=[None]):
        self.args = []
        self.counter = 1
        self.seed = 0
        for signal_type in signal_types:
            for w in lookback_windows:
                for t in thresholds:
                    for c in contravariants:
                        for idios in signal_idiosyncratic_paramaters:


                            self.args.append((self,self.seed,signal_type, w,t,c,json.dumps(idios, sort_keys=True)))
