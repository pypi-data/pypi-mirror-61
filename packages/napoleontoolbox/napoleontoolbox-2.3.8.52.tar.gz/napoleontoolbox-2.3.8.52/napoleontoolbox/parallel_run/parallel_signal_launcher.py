...
class SignalParalleLauncher():
    def __init__(self,drop_token='', dropbox_backup=True, local_root_directory='../data',user='napoleon',  db_path_suffix = '_run.sqlite', lookback_windows=[5], thresholds=[1.], contravariants=[1.]):
        self.args = []
        self.counter = 1
        self.seed = 0
        for w in lookback_windows:
            for t in thresholds:
                for c in contravariants:
                    self.args.append((self,self.seed,w,t,c))
    ...
    def addRun(self,run):
        ...
        try:
            sqliteConnection = sqlite3.connect(self.db_path)
            sqlite_create_table_query = 'alter table parallel_run add column ' + run + '_perf_return real, '+run+'_turn_over real'
            cursor = sqliteConnection.cursor()
            print("Successfully Connected to SQLite")
            cursor.execute(sqlite_create_table_query)
            sqliteConnection.commit()
    ...
    def insertResults(self, run, signal_results_df):
        sqliteConnection = None
        try:
            success = True
            sqliteConnection = sqlite3.connect(self.db_path)
