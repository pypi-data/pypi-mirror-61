...
def volume_weighted_high_low_vol(data = None , vol_threshold = 0.1, trend_threshold=0.02, contravariant = 1., **kwargs):
    trend = (data['close'][-1]/data['close'][0])/data['close'][0]
    data['hl'] = (data['high'] - data['low'])/data['low']
    data['volu_hi_low'] = data['volumefrom']*data['hl']
    volu_hi_low = data['volu_hi_low'].mean()
    if volu_hi_low > vol_threshold:
