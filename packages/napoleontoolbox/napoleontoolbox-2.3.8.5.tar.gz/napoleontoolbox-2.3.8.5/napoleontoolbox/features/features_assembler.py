...
def buildAdvancedPriceFeatures(quotes_df=None, max_timeshift=20, rolling_direction=1):
    strats = [col for col in quotes_df.columns if col != 'Date' ]
    aggregated_features = None
    for strat in strats:
        print('processing '+strat)
        df_shift, y = make_forecasting_frame(quotes_df[strat], kind="price", max_timeshift=20, rolling_direction=1)
...
class FeaturesAssembler(AbstractAssembler):
    ...
    def preprocessReturns(self):
        ...
        advanced_ts_features = buildAdvancedPriceFeatures(quotes_df=df[strats],max_timeshift=self.max_timeshift, rolling_direction=self.rolling_direction)
    ...
    def assembleFeature(self, feature_type, n_past_features):
        ...
        print('Final max predictor date ' + str(max(df_ret.index)))
