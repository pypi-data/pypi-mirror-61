...
class SimpleEnsemblingRunner(AbstractRunner):
    def runTrial(self, saver, seed,  n, s, low_bound, up_bound):
        ...
        if self.mdp_allocation and not check_mdp_existence:
            ...
            weight_normalized, last_predicted_weights = weights.process_weights(w=weight_mat.values, df=df_ret, s=s, n=n,
                                                                         low_bound=low_bound, up_bound=up_bound,
                                                                         normalize=True)
            portfolio = np.cumprod(np.prod(df_res * weight_mat.values + 1, axis=1))
            saver.saveResults('mdp_'+savingKey, portfolio, weight_normalized, None, None, last_predicted_weights)
