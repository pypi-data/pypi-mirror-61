...
class EcoVacsIOTMQ(ClientMQTT):
    ...
    def _disconnect(self):
        _LOGGER.error("Warning DISCONNECTED!!!")
    ...
    def __call_iotdevmanager_api(self, args, verify_ssl=True):
        ...
        try: #The RestAPI sometimes doesnt provide a response depending on command, reduce timeout to 3 to accomodate and make requests faster
            ...
            response = requests.post(url,headers=headers, json=params, timeout=30, verify=verify_ssl) #May think about having timeout as an arg that could be provided in the future
        ...
        if json['ret'] == 'ok':
            ...
            if 'debug' in json:
                if json['debug'] == 'wait for response timed out': 
                    ...
                    #LOGGER.warning("call to iotdevmanager failed with {}".format(json))
                    return {}
            else:
