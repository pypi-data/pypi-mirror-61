from typing import TypeVar, List, Any, Optional, Collection, Type

from django.core.paginator import Page
from django.forms import BaseForm
from django.utils.http import urlencode
from django_filters import FilterSet
from flex_forms.basic_mixins import TemplateObjectMixin, HttpRequestType
from flex_forms.components import BaseHtmlElement
from flex_forms.forms import StaticFieldset, StaticModelFieldset
...
class TableCell(AbstractTableElement, BaseHtmlElementWithSlots):
    """
...
class TableHeaderCell(AbstractTableElement, BaseHtmlElementWithSlots):
    """
...
TableCellType = TypeVar('TableCellType', TableCell, TableAdvancedCell)
TableHeaderCellType = TypeVar('TableHeaderCellType', TableHeaderCell, TableAdvancedCell)


class TableCaption(AbstractTableElement, BaseHtmlElementWithSlots):
    """
...
TableCaptionType = TypeVar('TableCaptionType', bound=TableCaption)
...
class BaseTableWrappingElement(BaseHtmlElementWithSlots):
    ...
    def __init__(self, data: List[AbstractTableElementType], css_classes: list = None, html_params: dict = None):
...
class TableRow(AbstractTableElement, BaseTableWrappingElement):
    """
...
class TableHeader(AbstractTableElement, BaseTableWrappingElement):
    """
...
TableHeaderType = TypeVar('TableHeaderType', bound=TableHeader)


class TableBody(AbstractTableElement, BaseTableWrappingElement):
    """
...
TableBodyType = TypeVar('TableBodyType', bound=TableBody)


class TableFooter(AbstractTableElement, BaseTableWrappingElement):
    """
...
TableFooterType = TypeVar('TableFooterType', bound=TableFooter)
TableFormObjectType = TypeVar('TableFormObjectType', FilterSet, BaseForm, StaticFieldset, StaticModelFieldset)
...
class BaseTemplateTable(TemplateObjectMixin):
    ...
    caption_class: Type[TableCaptionType] = TableCaption
    header_class: Type[TableHeaderType] = TableHeader
    body_class: Type[TableBodyType] = TableBody
    footer_class: Type[TableFooterType] = TableFooter
    row_class: Type[TableRowType] = TableRow
    body_cell_class: Type[TableCellType] = TableCell
    header_cell_class: Type[TableHeaderCellType] = TableHeaderCell

    _allowed_form_classes = (BaseForm, StaticFieldset, StaticModelFieldset)
    _elements: list = ['table_caption', 'table_header', 'table_body', 'table_footer']
    exclude: list = []

    def __init__(self, request: HttpRequestType, object_list: Collection, form_object: TableFormObjectType = None):
        self.request = request
        ...
        if isinstance(form_object, FilterSet):
            self.filter = form_object
            self.form = self.filter.form
        elif isinstance(form_object, self._allowed_form_classes):
    ...
    def get_context_data(self, **kwargs) -> dict:
        return super().get_context_data(
            **{self.context_object_name: self, 'filter': self.filter, 'form': self.form}, **kwargs
        )
...
class TemplateTablePagination(TemplateObjectMixin):
    ...
    def items_range(self) -> str:
        ...
        return f'{items_range} / Всего записей: {self.paginator.count}'
