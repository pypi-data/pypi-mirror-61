...
def draw_synchronous_product(input_net: AbstractPetriNet, filename="synchronous_product_net", format="pdf"):
	...
	for key, values in input_net.transitions.items():
		if key.endswith("_model"):
			for t in values:
				dot.node( "(" + key[:-6] + BLANK + ")", shape="rect", style='filled')
		elif key.endswith("_log"):
			for t in values:
				dot.node("(" + BLANK + key[:-4] +  ")", shape="rect", style='filled', color=color[1])
		elif key.endswith("_synchronous"):
			for t in values:
				dot.node( "(" + key[:-12] + key[-12] + ")", shape="rect", style='filled', color=color[2])
		...
		if e[0] > 0:
			for key, values in input_net.transitions.items():
				for t in values:
					if e[1] == t:
						if key.endswith("_synchronous"):
							dot.edge( str(e[0]), key + "(" + str(e[1]) + ")", color=color[1] )
						else:
		...
		#the goal of the edge is a place
			...
			for key, values in input_net.transitions.items():
				for t in values:
					if e[0] == t:
						if key.endswith("_synchronous"):
							dot.edge( key + "(" + str(e[0]) + ")", str(e[1]), color=color[1] )
						else:
