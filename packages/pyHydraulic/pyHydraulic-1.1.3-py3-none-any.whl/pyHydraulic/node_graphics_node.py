...
class QAbstractGraphicsNode(QGraphicsItem):
    def __init__(self, node):
        ...
        self.wasMoved = False
        # 下面可以添加一个通用变量
        self._name = self.node._node_type+ str(self.node.id) # 每个node有个独一无二的名字
        # 最大值最小值
        self._minValue = 0
    ...
    def value(self, value):
        if isinstance(value, int) or isinstance(value, float):
            if (value >= self._minValue) and (value <= self._maxValue):
                ...
                baseWidget = self.node.scene.grScene.views()[0]
                warningInfo = ("修改范围:%.2f ~ %.2f"%(self.minValue, self.maxValue))
    ...
    def maxValue(self, value):
        if value >= self.value:
            ...
            baseWidget = self.node.scene.grScene.views()[0]
            warningInfo = ("当前值必须保证在此范围:%.2f ~ %.2f" % (self.minValue, self.maxValue))
