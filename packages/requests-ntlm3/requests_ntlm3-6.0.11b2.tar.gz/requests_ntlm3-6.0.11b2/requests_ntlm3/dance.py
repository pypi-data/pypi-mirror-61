...
logger = logging.getLogger("requests_ntlm3")
...
class HttpNtlmContext(ntlm_auth.ntlm.NtlmContext):
    ...
    def __init__(
        ...
        logger.debug("Auth type: %s", auth_type)
        logger.debug("NTLM compatibility: %s", ntlm_compatibility)
    ...
    def get_negotiate_header(self):
        ...
        logger.debug("Negotiate header: %s", result)
        return result
