...
RB.ReviewRequestPage.IssueSummaryTableView = Backbone.View.extend({
    ...
    _noIssuesTemplate: _.template('<tr class="no-issues">\n <td colspan="5"><em><%- text %></em></td>\n</tr>'),
        ...
        if (this._$tbody.find('tr.issue').not('.hidden').length === 0) {
            ...
            if (this.reviewerFilterState !== 'all') {
                if (this.statusFilterState === 'open') {
                    text = interpolate(gettext('There are no open issues from %s'), [this.reviewerFilterState]);
                } else if (this.statusFilterState === 'verifying') {
                    text = interpolate(gettext('There are no issues waiting for verification from %s'), [this.reviewerFilterState]);
                } else if (this.statusFilterState === 'dropped') {
                    text = interpolate(gettext('There are no dropped issues from %s'), [this.reviewerFilterState]);
                } else if (this.statusFilterState === 'resolved') {
                    text = interpolate(gettext('There are no resolved issues from %s'), [this.reviewerFilterState]);
                }
            } else {
                if (this.statusFilterState === 'open') {
    ...
    _sortByCol: function _sortByCol(colIndex, ascending) {
        ...
        this._$tbody.html($('.issue').sort(function (issueA, issueB) {
            var $issueA = $(issueA);
            var $issueB = $(issueB);
            var $columnA = $issueA.children('td:nth-child(' + colIndex + ')');
            var $columnB = $issueB.children('td:nth-child(' + colIndex + ')');
            var value1 = void 0;
            var value2 = void 0;
     ...
     */
    _buildReviewerFilterMap: function _buildReviewerFilterMap() {
        var _this2 = this;

        _.each(this._$tbody.find('.issue'), function (issueEl) {
            var reviewer = $(issueEl).data('reviewer');

            if (!_.has(_this2.reviewerToSelectorMap, reviewer)) {
                _this2.reviewerToSelectorMap[reviewer] = '[data-reviewer="' + reviewer + '"]';
    ...
    _onIssueStatusChanged: function _onIssueStatusChanged(comment, oldStatus, timestamp) {
        ...
        $entry.removeClass(oldStatus).addClass(newStatus).find('.issue-icon').removeClass(this.statusIconsMap[oldStatus]).addClass(this.statusIconsMap[newStatus]);
        ...
        var $oldCounter = $('#' + oldStatus + '-counter');
        var $newCounter = $('#' + newStatus + '-counter');
        ...
        $entry.find('.last-updated time').attr('datetime', new Date(timestamp).toISOString()).text(timestamp).timesince();
