...
class CejCalculations:
    ...
    def __init__(self, bone_enamel_boundaries, tooth_mask, is_maxillary, np_image=None,
                 print_flag=False):
        """
        :param bone_enamel_boundaries: is a dict corresponding to boundary points for
            Bone and Enamel
        :param tooth_mask: is mask for a tooth
        :param is_maxillary: is where the tooth is maxillary
        """
        ...
        self.pixel_buffer = int(
            0.015 * np.sqrt(self.tooth_mask.size)
        )  # Ignore boundary points within pixel_buffer from image edge
