...
class CommunityEdition(object):
    ...
    def _get_params_for_events(self, api_name, start_time, end_time):
        ...
        total_events = response.json()["meta"]["total"]
        data["size"] = total_events
        response = requests.post(self.host_url + '/ce-api/v1/tenants/1000/events/search', headers=headers,
                                 data=json.dumps(data), verify=False)
        all_events_captured = {}
        is_api_secured = False
        for event in response.json()["data"]:
            all_events_captured[event["id"]] = event["attributes"]["event_json"].get("http-req-body-params", [])
            if event["attributes"]["event_json"].get("http-req-header-Authorization"):
                is_api_secured = True
        return all_events_captured, is_api_secured
    ...
    def _get_api_specific_details(self, groupid, start_time, end_time):
        ...
        jobs = []
        all_events_captured, is_api_secured = self._get_params_for_events(api_name, start_time, end_time)
