...
class _PipelineYamlParser(object):
    ...
    def load_yaml(workspace, filename, _workflow_provider=None, _service_endpoint=None):
        ...
        for step_name in steps_section:
            ...
            if _PARALLEL_RUN_STEP_TYPE == step_type or _PARALLEL_RUN_STEP in step:
                ...
                input_list = _PipelineYamlParser._get_input_datasets(workspace, step)
                output_list = list(outputs.values())
                ...
                # Due to parallelrunstep has not been move to pipeline step package, we can only import
                # here to avoid break others
    ...
    @staticmethod
    def _get_input_datasets(workspace, step):
        input_datasets = []
        if _INPUT_DATASETS in step:
            input_datasets_section = step[_INPUT_DATASETS]
            for input_ds_name in input_datasets_section.keys():
                if _DATASET_NAME in input_datasets_section[input_ds_name]:
                    dataset = Dataset.get_by_name(workspace=workspace,
                                                  name=input_datasets_section[input_ds_name][_DATASET_NAME])
                elif _DATASET_ID in input_datasets_section[input_ds_name]:
                    dataset = Dataset.get_by_id(workspace=workspace,
                                                id=input_datasets_section[input_ds_name][_DATASET_ID])
                input_datasets.append(dataset.as_named_input(input_ds_name))
