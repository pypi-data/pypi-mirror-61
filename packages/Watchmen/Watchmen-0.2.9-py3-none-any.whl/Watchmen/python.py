...
def compare_two_image(path1, path2):
    ...
    if os.path.exists('../Save Image'):
        ...
        os.mkdir('../Save Image')
        print('Folder /Save Image crated')
    if os.path.exists(path1) and os.path.exists(path2):
        ...
        if int(score) < 1.0:
            ...
            robotlib.fail('Save file with difference')
    else:
    ...
    if os.path.exists(path1):
        if os.path.exists(path2):
            ...
            if int(score) < 1.0:
                ...
                seleniumlib.capture_page_screenshot('../Save Image/img' + cas + '.png')
                cv.imwrite('../Save Image/img' + cas + '.png', img_diff)
                print("score" + str(score))
                print('Image has diff: {} %'.format(score_percen))
    ...
    if os.path.exists(path1):
        if os.path.exists(path2):
            ...
            if int(score) < 1.0:
                ...
                img_diff = cv.hconcat([img1, img2])
                cas = str(time.time())
                seleniumlib.capture_page_screenshot('../Save Image area/img' + cas + '.png')
                cv.imwrite('../Save Image area/img' + cas + '.png', img_diff)
                print("score" + str(score))
                score_percen = float(score) * + 100
                print("score" + str(score_percen))
        else:
            raise AssertionError("New screen doesnt exist anymore")
    else:
        raise AssertionError("You put bad path")
    if os.path.exists('../img/test_screen.png'):
        os.remove('../img/test_screen.png')
    ...
    if os.path.exists(path1) and os.path.exists(path2):
        ...
        if lt % 4 == 0:
            ...
            if int(score) < 1.0:
                ...
                img_diff = cv.hconcat([img1, img2])
                cas = str(time.time())
                seleniumlib.capture_page_screenshot('../Save Image areas/img' + cas + '.png')
                cv.imwrite('../Save Image areas/img' + cas + '.png', img_diff)
    else:
        raise AssertionError("Path doesnt exists")


# TODO Create keyword for take information from coordinate
def compare_screen_get_information(path1):
    ...
    Example: Compare screen ../image1.png
    """
    if os.path.exists('../watchmen/CSV_ERROR'):
        print('folder csv exists')
    else:
        os.mkdir('../watchmen/CSV_ERROR')
        print('Making csv')
    if os.path.exists('../watchmen/cs_get_info'):
        print('Folder save image exists')
    else:
        os.mkdir('../watchmen/cs_get_info')
    # Making screen
    seleniumlib = BuiltIn().get_library_instance('SeleniumLibrary')
    seleniumlib.capture_page_screenshot('../watchmen/cs_get_info/test_screen.png')
    path2 = '../watchmen/cs_get_info/test_screen.png'
    if os.path.exists(path1) and os.path.exists(path2):
        # load img
        img1 = cv.imread(path1, 1)
        img2 = cv.imread(path2, 1)
        ...
        # write coordinate
        with open('../watchmen/CSV_ERROR/bug_coordinates.csv', 'w') as csvfile:
        ...
        if int(score) < 1.0:
            ...
            img_diff = cv.hconcat([img1, img2])
            cas = str(time.time())
            seleniumlib.capture_page_screenshot('../watchmen/cs_get_info/img{0}.png'.format(cas))
            cv.imwrite('../watchmen/cs_get_info/img{0}.png'.format(cas), img_diff)

            # start reading coordinates and saving element from coordinate
            df = pd.read_csv(r'../watchmen/CSV_ERROR/bug_coordinates.csv')
            with open('../watchmen/CSV_ERROR/bug_co_and_name{0}.csv'.format(str(time.time())), 'w') as csv_name:
                ...
                for i in range(len(df)):
                    ...
                    elements = driver.execute_script("return document.elementsFromPoint(arguments[0], arguments[1]);", x_center, y_center)
