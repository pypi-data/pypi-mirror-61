...
def compare_screen_get_information(path1, save_folder="../cs_get_info", folder_csv='../CSV_ERROR'):
    ...
    if os.path.exists(path1) and os.path.exists(path2):
        ...
        if int(score) < 1.0:
            ...
            with open(folder_csv + '/bug_co_and_name{0}.csv'.format(str(time.time())), 'w') as csv_name:
                ...
                for i in range(len(df)):
                    ...
                    elements = driver.execute_script("return document.elementsFromPoint(arguments[0], arguments[1]);",
                                                     x_center, y_center)
